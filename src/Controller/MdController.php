...
class MdController extends AppController
{
    ...
    public function index()
    {
		...
		$user_list = $users_table->find()->where(array('role'=>'md'));
		$this->set('user_list',$user_list);
    }
	public function add($user_id=Null)
    {
		$users_table = TableRegistry::get('erp_users'); 
		$user_identy_id = $this->ERPfunction->generate_autoid('MD-');
		if(isset($user_id))
		{
			...
			$this->set('form_header','Edit M.D');
			$this->set('button_text','Update M.D');
		...
		else
		{
			...
			$this->set('form_header','Add M.D');
			$this->set('button_text','Add M.D');
		}
		...
		{	$this->set('user_data',$this->request->data);
			...
			{
				$this->Flash->success(__('MD Insert Successfully', null), 
							'default', 
							array('class' => 'success'));
				}
				}
			}
			$this->redirect(array("controller" => "Md","action" => "index"));		
		}		
    }
