 ...
 */
class PurchaseheadController extends AppController
{
    ...
    public function index()
    {
		...
		$user_list = $users_table->find()->where(array('role'=>'purchasehead'));
		$this->set('user_list',$user_list);
    }
	public function add($user_id=Null)
    {
		$users_table = TableRegistry::get('erp_users'); 
		$user_identy_id = $this->ERPfunction->generate_autoid('PH-');
		if(isset($user_id))
		{
			...
			$this->set('form_header','Edit Purchase Head');
			$this->set('button_text','Update Purchase Head');
		...
		else
		{
			...
			$this->set('form_header','Add Purchase Head');
			$this->set('button_text','Add Purchase Head');
		}
		...
		{	$this->set('user_data',$this->request->data);
			...
			{
				$this->Flash->success(__('Purchase Head Insert Successfully', null), 
							'default', 
							array('class' => 'success'));
				}
				}
			}
			$this->redirect(array("controller" => "Purchasehead","action" => "index"));		
		}		
    }
