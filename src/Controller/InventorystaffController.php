 ...
 */
class InventorystaffController extends AppController
{
    ...
    {
		$users_table = TableRegistry::get('erp_users'); 
		$user_identy_id = $this->ERPfunction->generate_autoid('INTSTAFF-');
		if(isset($user_id))
		{
			...
			$this->set('form_header','Edit Inventory Staff');
			$this->set('button_text','Update Inventory Staff');
		...
		else
		{
			...
			$this->set('form_header','Add Inventory Staff');
			$this->set('button_text','Add Inventory Staff');
		}
		...
		{	$this->set('user_data',$this->request->data);
			...
			{
				$this->Flash->success(__('Inventory Staff Insert Successfully', null), 
							'default', 
							array('class' => 'success'));
				}
				}
			}
			$this->redirect(array("controller" => "Inventorystaff","action" => "index"));		
		}		
    }
