<?php
	...
	$mpdf->WriteHTML("<td colspan='2' align='center'><strong>Amount</strong></td>");
	$mpdf->WriteHTML("</tr>");
	...
	foreach($wod_data as $retrive_material)
	{
			...
			$mpdf->WriteHTML("<td >{$this->ERPfunction->get_category_title($retrive_material['material_name'])} <br>
				({$retrive_material['detail_description']})
			</td>");
	...
	$mpdf->WriteHTML("<tr>");
	$mpdf->WriteHTML("<td colspan='7' style='text-align:center'><strong>Total Amount</strong></td>");
	$mpdf->WriteHTML("<td><strong>{$total_amount}</strong></td>");
	$mpdf->WriteHTML("<td><strong>{$total_amount_till_date}</strong></td>");
	$mpdf->WriteHTML("</tr>");
	if($cgst){
		$mpdf->WriteHTML("<tr>");
		$mpdf->WriteHTML("<td colspan='6' style='text-align:center'><strong>CGST(%)</strong></td>");
	...
	if($sgst){
		$mpdf->WriteHTML("<tr>");
		$mpdf->WriteHTML("<td colspan='6' style='text-align:center'><strong>SGST(%)</strong></td>");
	...
	if($igst){
		$mpdf->WriteHTML("<tr>");
		$mpdf->WriteHTML("<td colspan='6' style='text-align:center'><strong>IGST(%)</strong></td>");
	...
	$mpdf->WriteHTML("<tr>");
	$mpdf->WriteHTML("<td colspan='7' style='text-align:center'><strong>Net Amount</strong></td>");
