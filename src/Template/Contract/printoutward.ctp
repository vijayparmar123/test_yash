...
// $last_edit_by = isset($erp_grn_details['last_edit_by'])?$this->ERPfunction->get_user_name($erp_grn_details['last_edit_by']):'NA';
	...
	header('Content-Disposition: inline; filename="OUTWARD CORRESPONDENCE.pdf"');
	...
	$mpdf->WriteHTML($this->ERPfunction->viewheader_pdf($print_data['outward_date']));
	$mpdf->WriteHTML("<br>");
	...
	$mpdf->WriteHTML("<tbody>");
		...
		$mpdf->WriteHTML("<tr><td colspan='8' align='center'><h2><b>OUTWARD CORRESPONDENCE</b></h2></td></tr>");
		$mpdf->WriteHTML("<tr>");
			...
			$mpdf->WriteHTML("<tr>");
				$mpdf->WriteHTML("<td width=5px><b>Outward No:</b></td><td colspan='3'>{$print_data['our_outward_no']}</td>");
				$mpdf->WriteHTML("<td><b>Date:</b></td><td colspan='3'>{$this->ERPfunction->get_date($print_data['outward_date'])}</td>");
