	...
	<div class="col-md-12">
		...
		<?php echo $this->Form->Create('form1',['id'=>'eq_form','class'=>'form_horizontal formsize','method'=>'post','enctype'=>'multipart/form-data'],['url'=>['action'=>'']]);?>
				...
				$vendor_userid_a = array();
				 ...
				 $outward_from_date = isset($_POST['outward_from_date'])?$_POST['outward_from_date']:'';
				 $outward_to_date = isset($_POST['outward_to_date'])?$_POST['outward_to_date']:'';
			...
			?>
                    ...
                    <div class="content controls">
						<div class="form-row">
						<div class="col-md-2">Our Date From -</div>
                        <div class="col-md-4"><input type="text" name="outward_from_date" id="outward_from_date" value="<?php echo $outward_from_date;?>" class="datep form-control"/></div>
						<div class="col-md-2">Our Date To -</div>
                        <div class="col-md-4"><input type="text" name="outward_to_date" id="outward_to_date" value="<?php echo $outward_to_date;?>" class="datep form-control"/></div>
						</div>
                            ...
                            <div class="col-md-4">
								<select name="agency_client_name[]" class="select2"   style="width:100%;">
									...
									<option value="<?php echo $client_key ;?>" <?php 
												...
												if($client_key == $update_outward['agency_client_name']){
						...
						<div class="form-row">
							...
							<div class="col-md-2">Our outward No</div>
							<div class="col-md-4"><input name="out_outward_no" class="form-control"></div>				
						</div>
						<div class="form-row">
		...
		jQuery('#user_list').DataTable({responsive: true});
			...
			<table id="user_list"  class="dataTables_wrapper table table-striped table-hover">
				...
				<tbody>
					<?php
					$rows = array();
					$rows[] = array("Project Name","Our Outward No.","Our Outward Date","Their Ref. No.","Their Ref. Date","Agency Name","Subject");
						...
						?>
							<tr>								
								<td><?php echo ($export[] = $this->ERPfunction->get_projectname($outward_row['project_id']));?></td>
								<td><?php echo ($export[] = $outward_row['reference_no']);?></td>
								<td><?php echo ($export[] = $outward_row['date']->format("d-m-Y"));?></td>
								<td><?php echo ($export[] = $outward_row['our_outward_no']);?></td>
								<td><?php echo ($export[] = $outward_row['outward_date']->format("d-m-Y"));?></td>
								<td><?php echo ($export[] = $this->ERPfunction->get_category_title($outward_row['agency_name']));?></td>
								<!--<td><?php //echo ($export[] = $this->ERPfunction->get_agency_name($outward_row['agency_name']));?></td>-->
								<td><?php echo ($export[] = $outward_row['subject']);?></td>
								<td>
									<?php
								...
								<td>
								<?php 
								if($this->ERPfunction->retrive_accessrights($role,'viewoutwardlist')==1)
								{
								echo $this->Html->link("<i class='icon-eye-open'></i> View",array('action' => 'viewaddoutward', $outward_row['outward_id']),
								array('class'=>'btn btn-primary btn-clean','target'=>'_blank','escape'=>false));
								}
								if($this->ERPfunction->retrive_accessrights($role,'addoutward')==1)
								{
