<?php 
	...
	error_reporting(0);
		...
		header('Content-Disposition: inline; filename="subcontractrecords.pdf"');
		...
		$mpdf->WriteHTML('</head>');
		$mpdf->WriteHTML('<body>');	
		$mpdf->SetTitle('Sub Contract Records');	
		...
		$mpdf->WriteHTML("<td align=left><b>Sub Contract Records</b></td>");
		...
		unset($rows[0]);
		$header = array("Party Name","Bill Date","Bill No","Party's Name","Type of Bill","Type of Work","Gross Amount","Retention Money","Net Amount");
