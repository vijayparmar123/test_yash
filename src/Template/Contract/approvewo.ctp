...
<div class="row">
		...
		jQuery("body").on("click", "#first_approve_wo", function(event){
			...
			if(confirm('Are you Sure approve first step of this W.O.?'))
			{
                ...
                type:"POST",
						url:"<?php echo Router::url(array('controller'=>'Ajaxfunction','action'=>'firstapprovewo'));?>",
						data:curr_data,
						async:false,
			...
			if(confirm('Are you sure you want to verify this W.O.?'))
			{
                ...
                type:"POST",
						url:"<?php echo Router::url(array('controller'=>'Ajaxfunction','action'=>'verifyewo'));?>",
						data:curr_data,
						async:false,
						success: function(response){					
							 location.reload();
		...
		jQuery("body").on("click", "#approve_wo", function(event){
			...
			if(confirm('Are you Sure approve this W.O.?'))
			{
                ...
                type:"POST",
						url:"<?php echo Router::url(array('controller'=>'Ajaxfunction','action'=>'approvewo'));?>",
						data:curr_data,
						async:false,
						success: function(response){
	...
	<table id="wo_list"  class="dataTables_wrapper table table-striped table-hover">
		<thead>
			<tr>
				...
				<th>Action</th>
				<?php 
				if($this->ERPfunction->retrive_accessrights($role,'verifypurchasewoalert')==1)
				{
				?>
				<th>Verify</th>
				<?php
				}
				if($this->ERPfunction->retrive_accessrights($role,'approve1woalert')==1)
				{
				?>
				<th>Approve</th>
				<?php
				}
				if($this->ERPfunction->retrive_accessrights($role,'approve2woalert')==1)
				{
				?>
				<th>Final Approve</th>
				<?php
		...
		<tbody>
			<?php	
				...
				{	
				$first_step = $this->ERPfunction->get_wo_firststep_approve_value($retrive_data['wo_id']);
				$verified = $this->ERPfunction->get_wo_verified_value($retrive_data['wo_id']);
				?>
					<tr>								
						...
						<td>
						<?php 
						if($this->ERPfunction->retrive_accessrights($role,'approvewo')==1)
						{
							echo $this->Html->link("<i class='icon-eye-open'></i> View",array('action' => 'previewwo', $retrive_data['wo_id']),
							array('escape'=>false,'target'=>'blank','class'=>'btn btn-info btn-clean'));
							echo ' ';
						}
						if($this->ERPfunction->retrive_accessrights($role,'editpreparewo')==1)
						{
							echo $this->Html->link("<i class='icon-pencil'></i> Edit",array('action' => 'editpreparewo', $retrive_data['wo_id']),
							array('escape'=>false,'target'=>'blank','class'=>'btn btn-primary btn-clean'));
							echo ' ';
						}
						if($this->ERPfunction->retrive_accessrights($role,'deletewo')==1)
						{
							echo $this->Html->link("<i class='icon-pencil'></i> Delete",array('action' => 'deletewo', $retrive_data['wo_id']),
						...
						<div class='checkbox'>
							...
							<?php echo (($first_step==1) ? 'checked' : '');?> <?php echo (($verified==0 || $first_step==1) ? 'DISABLED' : '');?> data-id="<?php echo $retrive_data['wo_id']?>"/></label>
						</div>
						</td>
						<?php } ?>
