...
$allow_upload = ($this->ERPfunction->retrive_accessrights($role,'accountfileadd')==1)?true:false; // Set to true to allow upload files
...
if($_GET['do'] == 'list') {
	...
	if($type == 'dir')
	{
		...
		$from = $file;
		$to = str_replace("archive/","",$file);
		$alreadyHas = $filesystem->has($to);
		if($alreadyHas)
		{
			$loc = str_replace("/".$filename,"",$to);
			$file_details = explode(".",$filename);
			$name = $file_details[0];
			$ext =  $file_details[1];
			$copy_to = $loc."/".$name."-retrive.".$ext;
			$response = $filesystem->rename($from, $copy_to);
			//if($response)
			//{
				//$delete = $filesystem->delete($from);
			//}
		}else{
...
a.retrive {display:inline-block;
	background: url(data:image/png;base64,) no-repeat scroll 0 5px;
	color:#d00;	margin-left: 15px;font-size:11px;padding:0 0 0 13px;
}
.name {
...
$(function(){
	...
	$('#table').on('click','.retrive',function(data) {
		var type = $(this).attr('data-type');
...
<?php endif; ?>
	function list() {
		...
		<!-- } -->
		if(hashval != ''){
		$.get('../Ajaxfunction/getcloudfiles?file='+ hashval,function(data) {
			$tbody.empty();
	...
	function renderFileRow(data) {
		...
		<!-- var $retrive_link = $('<a/>').attr('href','?do=retrive&file='+ encodeURIComponent(data.path)) -->
			<!-- .addClass('retrive').text('Retrive'); -->
			
		var $retrive_link = $('<a href="#" data-type="'+data.type+'" file-name="'+data.name+'" />').attr('data-file',data.path).addClass('retrive').text('retrive');
		var perms = [];
		...
		var $html = $('<tr />')
			...
			.append( $('<td/>').append($dl_link).append( data.is_deleteable ? $delete_link : '').append($retrive_link) )
		return $html;
	}
...
<div class="content">
		...
		<?php echo $this->Form->Create('form15',['id'=>'user_form22','class'=>'form_horizontal formsize','method'=>'post','enctype'=>'multipart/form-data']);?>
					...
					<div class="form-row">
                            ...
                            <div class="col-md-4">
								<select class="select2" required style="width: 100%;" name="module_name" id="module_name">
                            ...
                            <div class="col-md-4">
								<select class="select2" required style="width: 100%;" name="project_id" id="project_id">
...
$(function(){
	...
	if(sub_location != '')
	{
		<!-- window.location.href = baseUrl+"/Usermanage/filemanagerbackup#account%2F"+sub_location; -->
		<!-- window.location.href = baseUrl+"/Usermanage/filemanagerbackup#archive%2F"+sub_location; -->
		window.location.href = baseUrl+"#archive%2F"+sub_location;
		return false;
	}else{
		<!-- window.location.href = baseUrl+"/Usermanage/filemanagerbackup#account"; -->
		<!-- window.location.href = baseUrl+"/Usermanage/filemanagerbackup#archive"; -->
