...
$last_edit_by = isset($erp_grn_details['last_edit_by'])?$this->ERPfunction->get_user_name($erp_grn_details['last_edit_by']):'NA';
	...
	header('Content-Disposition: inline; filename="debit_note.pdf"');
	...
	$mpdf->WriteHTML("<div style='width:100%;border-bottom:1 solid grey'><div id='left' style='width:80%;'>{$this->ERPfunction->viewheader_pdf($debit_list['date'])}</div><div id='right' style='width:20%;'><div style='height:50px;'></div><div>DEBIT NOTE</div></div></div>");
	...
	$mpdf->WriteHTML("<div style='width:100%;padding:5px 0;'><div width=10% id='debit'>Project :</div><div width=90% id='debit_name' style='border-bottom:1 solid grey;'>{$this->ERPfunction->get_projectname($debit_list['project_id'])}</div></div>");
	
	$mpdf->WriteHTML("<div>");
		$mpdf->WriteHTML("<div style='width:100%;'><div id='cv' style='width:50%;'><span>Debit Note NO : <u>{$debit_list['debit_note_no']}</u></span></div><div id='dt' style='width:30%;'><span>DATE : <u>{$this->ERPfunction->get_date($debit_list['date'])}</u></span></div></div>");
		
		 if(is_numeric($debit_list['debit_to']))
		 { 
			$party_name = $this->ERPfunction->get_vendor_name($debit_list['debit_to']);
		 }else { 
			$party_name = $this->ERPfunction->get_agency_name_by_code($debit_list['debit_to']);
		 } 
		
		$mpdf->WriteHTML("<div style='width:100%;padding:5px 0;'><div width=10% id='debit'>Debit to :</div><div width=90% id='debit_name' style='border-bottom:1 solid grey;'>{$party_name}</div></div>");
		
		$mpdf->WriteHTML("<div style='width:100%;padding:5px 0;'><div width=10% id='debit'>Paid to :</div><div width=90% id='debit_name' style='border-bottom:1 solid grey;'>{$debit_list['receiver_name']}</div></div>");
		
		$mpdf->WriteHTML("<div style='width:100%;padding:5px 0;'><div width=10% id='debit'>Reason :</div><div width=90% id='debit_name' style='border-bottom:1 solid grey;'>{$debit_list['reason']}</div></div>");
	...
	$mpdf->WriteHTML("<tbody>");
		...
		// $mpdf->WriteHTML("<tr>");
			...
			$mpdf->WriteHTML("<tr>");
				...
				$mpdf->WriteHTML("<td align='center' id='small'><b>Sr.No</b></td>");
				$mpdf->WriteHTML("<td align='center' colspan='5' id='big'><b>Reason</b></td>");
				$mpdf->WriteHTML("<td align='center' id='small'><b>Approx. Quantity</b></td>");
				$mpdf->WriteHTML("<td align='center' id='small'><b>Unit</b></td>");
				$mpdf->WriteHTML("<td align='center' id='small'><b>Approx. Rate</b></td>");
				//$mpdf->WriteHTML("<td  ></td>");
				$mpdf->WriteHTML("<td align='center' id='small'><b>Approx Amount(Rs.)</b></td>");
				//$mpdf->WriteHTML("<td  ></td>");
			...
			{	
				...
				$mpdf->WriteHTML("<tr>");
					...
					$mpdf->WriteHTML("<td align='center' colspan='5'>{$this->ERPfunction->get_material_title($retrive_material['material_id'])}</td>");
					$mpdf->WriteHTML("<td align='center'>{$retrive_material['quantity']}</td>");
					$mpdf->WriteHTML("<td align='center'>{$this->ERPfunction->get_category_title($this->ERPfunction->get_material_unit_id($retrive_material['material_id']))}</td>");
					$mpdf->WriteHTML("<td align='center'>{$retrive_material['rate']}</td>");
			...
			$i=0;
						...
						foreach($detail_list as $retrive_material){
							if(++$i === $numItems) 
							{
				 $mpdf->WriteHTML("<td colspan='7'>{$retrive_material['total_word']}</td>");
							}
						}
				 ...
				 $i=0;
						...
						foreach($detail_list as $retrive_material){
							if(++$i === $numItems) 
							{
				 $mpdf->WriteHTML("<td align='center'>{$retrive_material['total_amount']}</td>");
			...
			// $mpdf->WriteHTML("</tr>");
			 ...
			 $mpdf->WriteHTML("<tr>");
				$mpdf->WriteHTML("<td align='center' colspan='4'><br><br><br><br>");
						if($debit_list['created_by'])
						{
							$mpdf->WriteHTML("{$this->ERPfunction->get_user_name($debit_list['created_by'])}");
						}
				...
				$mpdf->WriteHTML("<h3><b> Prepared By </b></h3></td>");
				 $mpdf->WriteHTML("<td align='center' colspan='3'><br><br><br>");
						...
						foreach($detail_list as $retrive_material){
							if(++$i === $numItems) 
							{
								$mpdf->WriteHTML("{$this->ERPfunction->get_user_name($retrive_material['first_approved_by'])}"); 
							}
						}
				...
				$mpdf->WriteHTML("<h3><b> Received By </b></h3></td>");
