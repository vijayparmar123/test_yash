...
foreach($previw_list as $retrive_material)
{
	...
	$mpdf->WriteHTML("<td colspan='2' align='left'><b>Site Contact No:(1)</b></td>");
	$mpdf->WriteHTML("<td colspan='3' align='left'>{$this->ERPfunction->get_pr_contact($pr_mid,"one")}</td>");
	$mpdf->WriteHTML("<td colspan='4' align='left'><b>Site Contact No:(2)</b></td>");
	$mpdf->WriteHTML("<td colspan='2' align='left'>{$this->ERPfunction->get_pr_contact($pr_mid,"two")}</td>");
	$mpdf->WriteHTML("</tr>");
	...
	{		
		...
		$mpdf->WriteHTML("<td colspan='11' style='border-right:0'> 1.1 ) Loading & Transportation will be Paid Extra Amount (Rs.):  {$data['extra_transport']}</td>");
		$mpdf->WriteHTML("</tr>");
	}
	$mpdf->WriteHTML("<tr>");
	...
	$mpdf->WriteHTML("(Head - Purchase Department)</td>");
	$mpdf->WriteHTML("</tr>");
