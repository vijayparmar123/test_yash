...
$last_edit_by = isset($data['last_edit_by'])?$this->ERPfunction->get_user_name($data['last_edit_by']):'NA';
	...
	$mpdf->WriteHTML('</head>');
	$mpdf->WriteHTML('<body>');			
	$mpdf->WriteHTML($this->ERPfunction->viewheader_pdf($data['rbn_date']));

	    
	$mpdf->WriteHTML("<table width=100%  >");
	$mpdf->WriteHTML("<tbody>");
	$mpdf->WriteHTML("<tr><td colspan='7' align='center' style='border-right:0'><h2><strong color=gray>Returned Back Note (RBN)</strong></h2></td></tr>");
	$mpdf->WriteHTML("<tr>");
	...
	$mpdf->WriteHTML("</tr>");	
			...
			$mpdf->WriteHTML("<td style='border-right:0;' colspan='5' align='center'><h3><strong>Approved By </strong></h3><br><br><br><br><br><br>");
			$approver = array();
			$ids = array();
						foreach($previw_list as $retrive_material){
