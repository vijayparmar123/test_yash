<?php
	...
	$mpdf->WriteHTML("<td colspan='12' align='center'><strong>Work / Item</strong></td>");
	// $mpdf->WriteHTML("<td width=60px rowspan='2'><strong>Amount (Inclusive <br> All)</strong></td>");
	$mpdf->WriteHTML("</tr>");
	
	$mpdf->WriteHTML("<tr>");
	$mpdf->WriteHTML("<td colspan='5'><strong>Work Head</strong></td>");
	$mpdf->WriteHTML("<td colspan='5'><strong>Description</strong></td>");
	$mpdf->WriteHTML("<td><strong>Quantity</strong></td>");
	$mpdf->WriteHTML("<td><strong>Unit</strong></td>");
	...
	foreach($wod_data as $retrive_material)
	{
			...
			$mpdf->WriteHTML("<tr>");
			$mpdf->WriteHTML("<td>{$retrive_material['contract_no']}</td>");
			$mpdf->WriteHTML("<td colspan='5'>{$this->ERPfunction->get_work_head_title($retrive_material['work_head'])}</td>");
			$mpdf->WriteHTML("<td colspan='5'>{$retrive_material['material_name']}</td>");
			$mpdf->WriteHTML("<td>{$retrive_material['quentity']}</td>");
			$mpdf->WriteHTML("<td>{$retrive_material['unit']}</td>");
	...
	$mpdf->WriteHTML("<tr>");
	$mpdf->WriteHTML("<td colspan='12' style='border-right:0'> 4) Material/item supplied must meet IS specifications; on failing to match with it or will found unsatisfactory after some days of delivery; supplier/party has to replace that free of cost and this PO will be considered as void.</td>");
	$mpdf->WriteHTML("</tr>");
	...
	$mpdf->WriteHTML("<tr>");
	if($wo_data["bill_mode"] == "gujarat" || $wo_data["bill_mode"] == "maharastra"){
	$mpdf->WriteHTML("<td colspan='12' style='border-right:0'> <strong>Billing Address:</strong>214/5, Khyati Complex, Near Mithakhali Underbridge, Ellisbridge, Ahmedabad - 380006,Gujarat</td>");
	}else{
	$mpdf->WriteHTML("<td colspan='12' style='border-right:0'> <strong>Billing Address:</strong>{$this->ERPfunction->getmpbilladdress($wo_data["wo_date"])}</td>");
	}
