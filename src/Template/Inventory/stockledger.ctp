...
<div class="col-md-10" >
	...
	<div class="row">
		<div class="col-md-12">
			...
			<script>
				jQuery(document).ready(function() {
					var table = jQuery('.stockledger_table').DataTable({
						responsive: {						
						},
						aaSorting: false
					});
					...
					<?php
						...
						if(!empty($stockledger)) {	
							...
							foreach($stockledger as $retrive_data) {
								...
								$vendorName = "";
								if(isset($desc_code['vendor_userid'])) {
									$vendorName = $this->ERPfunction->get_vendor_name($desc_code['vendor_userid']);
								}else {
									$vendorName = "-";
								}
								// debug($vendorName);
							...
							<tr> 
								...
								<td>
									<?php echo ($csv[] = ($retrive_data['material_id']!= 0)?$this->ERPfunction->get_items_units($retrive_data["material_id"]):$retrive_data['static_unit']);
									?>
								</td>
								<?php 
