<?php 
	...
	error_reporting(0);
		...
		header('Content-Disposition: inline; filename="WOnorate.pdf"');
		...
		$mpdf->SetTitle('WO Records');	
		$mpdf->WriteHTML('<table width=100%>');
		$mpdf->WriteHTML("<tr align=left>");
		$mpdf->WriteHTML("<td align=left><b>WO Records</b></td>");
		...
		unset($rows[0]);
		$header = array("#","Date","W.O. No","Project Name","Party Name","Type of Contract");
