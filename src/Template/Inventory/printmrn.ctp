<?php
	...
	$mpdf->WriteHTML('</head>');
	$mpdf->WriteHTML('<body>');	
	$mpdf->WriteHTML($this->ERPfunction->viewheader_pdf($data['mrn_date']));
	$mpdf->WriteHTML("<br>");
	...
	$mpdf->WriteHTML("<tr>");
			...
			$mpdf->WriteHTML("<td style='border-right:0;' colspan='4' align='center'><h3><strong> Approved By </strong></h3><br><br><br>&nbsp;{$this->ERPfunction->get_user_name($data['approve_executives'])}&nbsp;</td>");
			$mpdf->WriteHTML("</tr>");
