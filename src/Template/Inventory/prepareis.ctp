...
jQuery(document).ready(function() {
	 	 ...
	 	 jQuery.ajax({
			headers: {
                ...
                success: function(response){					
					...
					jQuery('select.asst_list').append(json_obj['assets']);	
  ...
  jQuery('body').on('click','.trash',function(){
	...
	jQuery("body").on("change", "#is_date", function(event){
		var project_id = jQuery("#project_id").val();
		var date = jQuery("#is_date").val();
                ...
                <div class="block block-fill-white">
					...
					<input type="hidden" name="user_action" class="form-control" value="<?php echo $user_action;?>"/>	
					 ...
					 <div class="content controls">
						...
						<div class="form-row">
							<!-- IS No created after form submit so hide this field here, here was problem with creation duplicate IS No. -->
                            <!--<div class="col-md-2">I.S.No</div>
                            <div class="col-md-4">
								<input type="text" name="is_no" id="is_no" value="" class="form-control" value=""/>
							</div>-->
                        
                            <div class="col-md-2">Date*</div>
                            <div class="col-md-2"><input type="text" onkeydown="return false" name="is_date" id="is_date" 
							value="<?php //echo $this->ERPfunction->get_date(date('Y-m-d'));?>" class="form-control validate[required]" value=""/></div>
							
                        </div>						
						<div class="form-row">
                            <div class="col-md-2" style="padding: 0;">Vendor Name/Asset Name*</div>
                            <div class="col-md-10">
								<?php  echo $this->Form->select("agency_name",$vendor_assets,["empty"=>" ","class"=>"select2 asst_list","id"=>"","style"=>"width:100%;","required"=>true]);?>
							</div>                        
                        </div>
						<!-- <div class="form-row">
                            ...
                            <table class="table table-bordered">
								...
								<tbody>
									<tr id="row_id_0">
										...
										<td><span id="opening_stock_0"></span></td>
										<td><input type="text" name="material[quantity][]" id="quantity_0" value="" class="form-control change_bal validate[required]" row="0" /></td>
										<td><input type="text" name="material[balance][]" id="balance_0" class="form-control" style="padding:0;" readonly /></td>
										...
										<td><input type="text" name="material[time_issue][]" id="time_issue_0" value="" class="form-control" /></td>
										<td>
											<span class="trash btn btn-danger" data-id="0" ><i class="icon-trash"></i></span>
										</td>
									</tr>
								</tbody>
							</table>
...
jQuery(document).ready(function(){
	...
	jQuery("body").on("click","#generate",function(){
			if(confirm("Are you sure,you want to Preapare IS?"))
			{
				if(confirm("Are you sure,you want to Preapare IS?"))
				{
					return true;
