...
foreach($previw_list as $retrive_material)
{
	...
	$mpdf->WriteHTML("<tr>");
	$mpdf->WriteHTML("<td align='left'><b>Place of Delivery : </b></td><td colspan='12'>{$this->ERPfunction->get_projectaddress($data['project_id'])}</td>");
	$mpdf->WriteHTML("</tr>");
	...
	$mpdf->WriteHTML("<tr>");
	$mpdf->WriteHTML("<td align='left' colspan='1'><b>Payment Method</b></td><td colspan='9'>{$data['payment_method']}</td>");
	...
	$mpdf->WriteHTML("<tr>");
	$mpdf->WriteHTML("<td colspan='12' style='border-right:0'> 2) If material/item supplied must meet IS specifications; on failing to match with it or will found unsatisfactory after some days of delivery; supplier/party has to replace that free of cost and this PO will be considered as void.</td>");
	$mpdf->WriteHTML("</tr>");
	...
	$mpdf->WriteHTML("<tr>");
	$mpdf->WriteHTML("<td colspan='12' style='border-right:0'> 8) For payment party will have to submit <strong> Invoice along with Purchase Order (PO), Gate Pass - Goods Inward, Goods Receipt Note or/and Rejection Memo and/or Weight Pass.</strong></td>");
	$mpdf->WriteHTML("</tr>");
	
	$mpdf->WriteHTML("<tr>");
	if($data["bill_address"] == "gj"){
	$mpdf->WriteHTML("<td colspan='12' style='border-right:0'> <strong>Billing Address: </strong>214/5, Khyati Complex, Near Mithakhali Underbridge, Ellisbridge, Ahmedabad - 380006.</td>");
	}else{
	$mpdf->WriteHTML("<td colspan='12' style='border-right:0'> <strong>Billing Address: </strong>214/5, Khyati Complex, Near Mithakhali Underbridge, Ellisbridge, Ahmedabad - 380006.</td>");
	}
	$mpdf->WriteHTML("</tr>");
	...
	$mpdf->WriteHTML("<td align='center' style='border-bottom:0;' valign='bottom' colspan='6' height='80px'>");
						...
						foreach($previw_list as $retrive_material)
						{
							...
							{
								$mpdf->WriteHTML("<font size='4'>{$this->ERPfunction->get_user_name($retrive_material['first_approved_by'])}</font><br>");
								break;
							}
						}
