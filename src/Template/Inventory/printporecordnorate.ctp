<?php
	...
	foreach($previw_list as $retrive_material)
	{
			...
			$mpdf->WriteHTML("<td colspan='3'>{$mt}</td>");
	...
	$mpdf->WriteHTML("<tr>");
	$mpdf->WriteHTML("<td rowspan='14' valign='top' style='border-bottom:0;'><strong> Remarks/Note:</strong></td>");
	...
	$mpdf->WriteHTML("<tr>");
	$mpdf->WriteHTML("<td colspan='12' style='border-right:0'> 2) If material/item will found unsatisfactory after some days of delivery; supplier/party has to replace that.</td>");
	$mpdf->WriteHTML("</tr>");
	...
	$mpdf->WriteHTML("<tr>");
	$mpdf->WriteHTML("<td colspan='12' style='border-right:0'> 4) No Extra Charge will be paid for waiting.</td>");
	$mpdf->WriteHTML("</tr>");
	
	$mpdf->WriteHTML("<tr>");
	$mpdf->WriteHTML("<td colspan='12' style='border-right:0'> 5) For payment party will have to submit <strong> Invoice along with Purchase Order (PO), Gate Pass - Goods Inward, Goods Receipt Note or/and Rejection Memo and/or Weight Pass.</strong></td>");
	$mpdf->WriteHTML("</tr>");
	...
	$mpdf->WriteHTML("<tr>");
	$mpdf->WriteHTML("<td colspan='12' style='border-right:0'> 6) YashNand has right to cancel order any time.</td>");
	$mpdf->WriteHTML("</tr>");
	
	$mpdf->WriteHTML("<tr>");
	$mpdf->WriteHTML("<td colspan='12' style='border-right:0'> 7) Payment will be done <strong>{$data['payment_days']}</strong> days after date of delivery on site or bill submission which ever is later.</td>");
	$mpdf->WriteHTML("</tr>");
