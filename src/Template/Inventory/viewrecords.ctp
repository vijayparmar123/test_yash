...
multiple="multiple">
					...
					<div class="form-row">
						<div class="col-md-2">Material Sub Group</div>
						<div class="col-md-3">
							<select class="select2" style="width: 100%;" name="material_sub_group[]" id="material_sub_group" multiple="multiple">
							<option value="All">All</Option>
							<?php 
								foreach($material_sub_group as $retrive_data)
								{
									echo '<option value="'.$retrive_data['sub_group_id'].'">'.
									$retrive_data['sub_group_title'].'</option>';
								}
							?>
							</select>
						</div>
...
<input type="hidden" id="f_material_sub_group" value="<?php echo isset($_POST["material_sub_group"]) ? implode(",",$_POST["material_sub_group"]) : "";?>">
...
<input type="hidden" id="f_cost_group" value="<?php echo isset($_POST["cost_group"]) ? implode(",",$_POST["cost_group"]) : "";?>">
		...
		jQuery(document).ready(function() {
			...
			var f_material_sub_group  = jQuery("#f_material_sub_group").val();
		...
		var table = jQuery('#mrn_list').DataTable({
			...
			"ajax": {
				...
				'X-CSRF-Token': csrfToken
					...
					"url": "../Ajaxfunction/inventoryrecords",
					"data": function ( d ) {
												d.myKey = "myValue";
												d.pro_id = f_pro_id;
												d.material_sub_group = f_material_sub_group;
	 	 ...
	 	 jQuery.ajax({
			...
			<div class="content">
				...
				<div class="col-md-2">
					...
					<input type="hidden" name="f_material_sub_group" value="<?php echo isset($_POST["material_sub_group"]) ? implode(",",$_POST["material_sub_group"]) : "";?>">
