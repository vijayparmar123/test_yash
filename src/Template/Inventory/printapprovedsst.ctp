<?php
	...
	$mpdf->WriteHTML("<tr>");
	$mpdf->WriteHTML("<td rowspan='2' align='center'><strong> Material Code : </strong></td><td colspan='5' align='center'><strong>Material / Item</strong></td>");
	...
	foreach($previw_list as $retrive_material)
	{
			...
			$mpdf->WriteHTML("<td style='border-right:#333;'>{$retrive_material['intimated_by']}</td>");
			$mpdf->WriteHTML("</tr>");
	}
	...
	$mpdf->WriteHTML("<td colspan='4' align='center' height='120px' valign='top' style='border-right:#333;'><h3><strong> Approved By </strong></h3><br><br><br>");
	$approver = array();
			$ids = array();
						foreach($previw_list as $retrive_material){
