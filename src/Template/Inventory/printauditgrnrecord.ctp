...
// $approve_on = isset($erp_grn_details['created_date'])?$erp_grn_details['created_date']:'NA';
$created_on = date("d-m-Y H:i:s",strtotime($erp_grn_details['created_date']));
	...
	$mpdf->WriteHTML("<tbody>");
		...
		$mpdf->WriteHTML("<tr><td colspan='8' align='center'  style='border-right:0;border-top:1 solid'><h2><b>Goods Receipt Note (GRN)</b></h2></td></tr>");
		$mpdf->WriteHTML("<tr>");
			...
			}elseif($erp_grn_details['local_po_id']!=""){
				$mpdf->WriteHTML("<td><b>Local P.O. No:</b></td>
					<td colspan='2' >
					{$this->ERPfunction->get_pono_by_grnid($erp_grn_details['grn_id'])}
					</td>");
				$mpdf->WriteHTML("<td colspan='2' ></td><td colspan='3' style='border-right:0;'></td>");
			}else{
				$manual_po_no = ($erp_grn_details['manualpo_no'])?$erp_grn_details['manualpo_no']:'NA';
				$mpdf->WriteHTML("<td><b>Manual P.O. No:</b></td>
					<td colspan='2' >
					".$manual_po_no."
					</td>");
				$mpdf->WriteHTML("<td colspan='2' ></td><td colspan='3' style='border-right:0;'></td>");
			...
			{
			$mpdf->WriteHTML("<tr><td colspan='8' align='center' style='border-right:0;'><font size='4'><b>If Paid in Cash Enter following information</b></font></td></tr>");
			$mpdf->WriteHTML("<tr>");
			...
			$mpdf->WriteHTML("<tr>");
				$mpdf->WriteHTML("<td width=300 align=center><b>Description</b></td>");
			...
			{	
				...
				$mpdf->WriteHTML("<tr>");
					...
					$mpdf->WriteHTML("<td>".bcdiv($retrive_material['quantity'],1,3)."</td>");
					$mpdf->WriteHTML("<td>".bcdiv($retrive_material['actual_qty'],1,3)."</td>");
			...
			// $mpdf->WriteHTML("<tr>");
				...
				// $mpdf->WriteHTML("<td align='center' style='border-right:0;' colspan='5'><br><br><br> (Construction Manager)</td>");
			// $mpdf->WriteHTML("</tr>");
