...
<?php 
	...
	error_reporting(0);
		...
		header('Content-Disposition: inline; filename="po_delivery_records.pdf"');
		...
		$mpdf->WriteHTML('</head>');
		$mpdf->WriteHTML('<body>');	
		$mpdf->SetTitle('PO Delivery Records');	
		$mpdf->WriteHTML($this->ERPfunction->viewheader_pdf());
	
		$mpdf->WriteHTML('<table width=100%>');
		$mpdf->WriteHTML("<tr align=left>");
		$mpdf->WriteHTML("<td align=left style='border-right:0'><b>PO Delivery Records</b></td>");
		...
		unset($rows[0]);
		$header = array("P.O. No","P.O.Date","Project Name","Vendor Name","Material Name","Make/Source","PO Quantity","Received Quantity","PO's Remaining Quantity","Unit","Remarks");
