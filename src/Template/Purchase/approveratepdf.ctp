...
<?php 
	...
	error_reporting(0);
		...
		header('Content-Disposition: inline; filename="approverate_list.pdf"');
		...
		$mpdf->WriteHTML('</head>');
		$mpdf->WriteHTML('<body>');	
		$mpdf->SetTitle('Approve Rate List');	
		$mpdf->WriteHTML($this->ERPfunction->viewheader_pdf());
	
		$mpdf->WriteHTML('<table width=100%>');
		$mpdf->WriteHTML("<tr align=left>");
		$mpdf->WriteHTML("<td align=left style='border-right:0'><b>Approve Rate List</b></td>");
		...
		unset($rows[0]);
		$header = array("Date From","Date To","Project Name","Vendor Name","Material Name","Final Rate","Unit","All Taxes & Duties","Loading & Transportation (F.O.R)","Unloading");
