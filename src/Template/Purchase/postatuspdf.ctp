...
<?php 
	...
	error_reporting(0);
		...
		header('Content-Disposition: inline; filename="po_status.pdf"');
		...
		$mpdf->WriteHTML('</head>');
		$mpdf->WriteHTML('<body>');	
		$mpdf->SetTitle('PO Status Records');	
		$mpdf->WriteHTML($this->ERPfunction->viewheader_pdf());
	
		$mpdf->WriteHTML('<table width=100%>');
		$mpdf->WriteHTML("<tr align=left>");
		$mpdf->WriteHTML("<td align=left style='border-right:0'><b>PO Status Records</b></td>");
