...
foreach($previw_list as $retrive_material)
{
	...
	$mpdf->WriteHTML("<tr>");
	$mpdf->WriteHTML("<td colspan='11' align='center'><h2><strong><u>Letter of Intent</u> (<u>LOI</u>)</strong></u></h3></td>");
	$mpdf->WriteHTML("</tr>");
	...
	$mpdf->WriteHTML("<tr>");
	$mpdf->WriteHTML("<td align='left'><b>LOI No. : </b></td><td colspan='2'>{$data['loi_no']}</td>");
	$mpdf->WriteHTML("<td colspan='2' align='left'><b>Date : </b></td><td colspan='3'>".date("d-m-Y",strtotime($data['loi_date']))."</td>");
	$mpdf->WriteHTML("<td colspan='1' align='left'><b>Time : </b></td><td colspan='2'>{$data['loi_time']}</td>");
	$mpdf->WriteHTML("</tr>");
