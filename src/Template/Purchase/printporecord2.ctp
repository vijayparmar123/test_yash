...
foreach($previw_list as $retrive_material)
{
	...
	$mpdf->WriteHTML("<tr>");
	$mpdf->WriteHTML("<td colspan='12' align='center'><h2><strong><u>Purchase Order</u> (<u>P.O.</u>)</strong></u></h3></td>");
	$mpdf->WriteHTML("</tr>");
	
	$mpdf->WriteHTML("<tr>");
	$mpdf->WriteHTML("<td><b>P. O. No. : </b></td>");
	$mpdf->writeHTML("<td colspan='1'>{$data['po_no']}</td>");
	$mpdf->WriteHTML("<td align='left'><b>Date : </b></td>");
	$mpdf->writeHTML("<td colspan='7'>".date("d-m-Y",strtotime($data['po_date']))."</td>");

	$mpdf->WriteHTML("</tr>");
	
	$mpdf->WriteHTML("<tr>");
	$mpdf->WriteHTML("<td align='left'><b>Project Name : </b></td>");
	$mpdf->WriteHTML("<td colspan='9'>{$this->ERPfunction->get_projectname($data['project_id'])}</td>");
	...
	$mpdf->WriteHTML("<tr>");
	$mpdf->WriteHTML("<td align='left'><b>P.O. Type : </b></td>");
	$mpdf->WriteHTML("<td>{$type_name}</td>");
	$mpdf->WriteHTML("<td align='left'><b>Project Code : </b></td>");
	$mpdf->writeHTML("<td colspan='9'>{$this->ERPfunction->get_projectcode($data['project_id'])}</td>");
	...
	$mpdf->WriteHTML("<tr>");
	$mpdf->WriteHTML("<td align='left'><b>Billing Mode : </b></td>
		<td>{$bill_mode}</td>");
	$mpdf->WriteHTML("<td align='left'><b>Usage : </b></td>");
	$mpdf->WriteHTML("<td colspan='9' align='left'>".ucfirst(str_replace('_',' ',$data['usage_name']))."</td>");
	$mpdf->WriteHTML("</tr>");
	...
	$mpdf->WriteHTML("<tr>");
	$mpdf->WriteHTML("<td align='left'><b>Vendor Name : </b></td><td>{$this->ERPfunction->get_vendor_name($data['vendor_userid'])}</td>");
	$mpdf->WriteHTML("<td align='left'><b>Vendor ID : </b></td>");
	$mpdf->WriteHTML("<td align='left'>{$data['vendor_id']}</td>");
	$mpdf->WriteHTML("</tr>");
	...
	$mpdf->WriteHTML("<tr>");
	$mpdf->WriteHTML("<td align='left'><b>PAN No</b></td><td>".$this->ERPfunction->get_vendor_detail($data['vendor_userid'],'pancard_no')."</td>");
	$mpdf->WriteHTML("<td align='left'><b>GST No</b></td>");
	$mpdf->WriteHTML("<td colspan='9' align='left'>".$this->ERPfunction->get_vendor_detail($data['vendor_userid'],'gst_no')."</td>");
	$mpdf->WriteHTML("</tr>");
	
	$mpdf->WriteHTML("<tr>");
	$mpdf->WriteHTML("<td align='left'><b> Contact No:(1)</b></td><td>{$this->ERPfunction->get_vendor_contact($data['vendor_userid'],'one')}</td>");
	$mpdf->WriteHTML("<td align='left'><b>Contact No:(2)</b></td>");
	$mpdf->WriteHTML("<td colspan='9' align='left'>{$this->ERPfunction->get_vendor_contact($data['vendor_userid'],'two')}</td>");
	$mpdf->WriteHTML("</tr>");
	...
	$mpdf->WriteHTML("<td colspan='1' align='left'>{$data['contact_no1']}</td>");
	$mpdf->WriteHTML("<td align='left'><b>Site Contact No:(2)</b></td>");
	$mpdf->WriteHTML("<td colspan='9' align='left'>{$data['contact_no2']}</td>");
	$mpdf->WriteHTML("</tr>");
	
	 $delivery_date =  ($data['delivery_date'] != NULL)?date("d-m-Y",strtotime($data['delivery_date'])):""; 
	
	$mpdf->WriteHTML("<tr>");
	$mpdf->WriteHTML("<td align='left'><b>Payment Method</b></td><td colspan='1'>{$data['payment_method']}</td>");
	$mpdf->WriteHTML("<td align='left' colspan='1'><b>Mode of GST:</b></td><td colspan='9'>{$data['mode_of_gst']}</td>");
	...
	$mpdf->WriteHTML("<td rowspan='2' style='width:75px;' align='center'><strong>Amount (Inclusive All)</strong></td>");
	// $mpdf->WriteHTML("<td rowspan='2'><strong>Final Rate<br>(Inclusive All)</strong></td>");
	$mpdf->WriteHTML("</tr>");
	
	$mpdf->WriteHTML("<tr>");
	$mpdf->WriteHTML("<td colspan='4' align='center' style='max-width:400px;'><strong>Description</strong></td>");
	// $mpdf->WriteHTML("<td><strong>HSN Code</strong></td>");
	$mpdf->WriteHTML("<td width='55px'><strong>Make / Source</strong></td>");
	$mpdf->WriteHTML("<td width='35px'><strong>Quantity</strong></td>");
	$mpdf->WriteHTML("<td width='35px'><strong>Unit</strong></td>");
	// $mpdf->WriteHTML("<td><strong>Unit Rate (including Loading, Unloading, Transport and All Taxes) (Rs.)</strong></td>");
	$mpdf->WriteHTML("<td width='35px'><strong>Unit Rate<br>(Rs.)</strong></td>");
	$mpdf->WriteHTML("<td width='35px'><strong>Dis(%)</strong></td>");
	// $mpdf->WriteHTML("<td><strong>CGST(%)</strong></td>");
	// $mpdf->WriteHTML("<td><strong>SGST(%)</strong></td>");
	// $mpdf->WriteHTML("<td><strong>IGST(%)</strong></td>");
	$mpdf->WriteHTML("<td width='35px'><strong>GST(%)</strong></td>");
	$mpdf->WriteHTML("</tr>");
	...
	foreach($previw_list as $retrive_material)
	{
			...
			$mpdf->WriteHTML("<td colspan='4'><center>(".$m_code.")<br>".$mt."
			<br>(".$retrive_material['description'].")
			</center></td>");
			...
			// $mpdf->WriteHTML("<td>{$retrive_material['other_tax']}</td>");
				$mpdf->WriteHTML("<td>{$retrive_material['gst']}</td>");
			$mpdf->WriteHTML("<td align='center'>{$retrive_material['amount']}</td>");
	...
	$mpdf->WriteHTML("<td align='center'><strong>{$total_amount}</strong></td>");
	...
	$mpdf->WriteHTML("<tr>");
	// $mpdf->WriteHTML("<td align='center' style='border-bottom:0;' valign='bottom' colspan='".(($is_mail == 0)?3:8)."' height='80px'>
	...
	$mpdf->WriteHTML("<td align='center' style='border-bottom:0;' valign='bottom' colspan='6' height='80px'><font size='4'><strong>{$this->ERPfunction->getletterheadsign($data['po_date'],$data['bill_mode'])}</strong></font><br><br>");
						foreach($previw_list as $retrive_material)
						{
							if($retrive_material['verified_by'])
							{
