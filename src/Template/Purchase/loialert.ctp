...
?>
			...
			echo $this->Form->Create('form2',['id'=>'app_frm','method'=>'post','url'=>['controller'=>"Purchase",'action'=>'showinloirecords']]);
			?>
			...
			<table id="pr_list"  class="table table-bordered table-striped table-hover">
					...
					<tr>
						...
						<th colspan="9" style="padding:0;">
							<table class='table-bordered' style='width:100%;'>
								...
								<th>Edit/View</th>
								<?php
								if($this->ERPfunction->retrive_accessrights($role,'verifyloi')==1)
								{
								?>
								<th>Verify</th>
								<?php
								}
								if($this->ERPfunction->retrive_accessrights($role,'approve1loi')==1)
								{
								?>
								<th>Approve</th>
								<?php
								}
								if($this->ERPfunction->retrive_accessrights($role,'approve2loi')==1)
								{
								?>
								<th>Final Approve</th>
								<?php
				...
				<tbody>
					...
					echo $this->ERPfunction->get_loi_alerts($selected_project);
					?>
					<input type="hidden" name="loi" id="po_text">
				</tbody>
			</table>
			</div>
			<?php 
