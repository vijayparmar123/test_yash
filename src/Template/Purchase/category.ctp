			...
			<table id="category_list"  class="dataTables_wrapper table table-striped table-hover">
				...
				<tbody>
					...
					if(!empty($category)){
						...
						foreach($category as $retrive_data)
							{ ?>
							<tr>								
								<td><?php echo $retrive_data['material_code'];?></td>	
								<td><?php echo $retrive_data['material_cat_name'];?></td>	
								<td><?php echo $this->Html->link(__('View Material'),array('action' => 'viewmaterial',$retrive_data['material_cat_id']),
								array('class'=>'btn btnview btn-info')); ?> 
								</td>	
							</tr>	
						<?php /* $i = 1;
