			...
			jQuery(document).ready(function() {
				...
				var table = jQuery('#po_list').DataTable({
					...
					"ajax": {
						...
						url:"<?php echo Router::url(array('controller'=>'Ajaxfunction','action'=>'poammendrecords'));?>",
						"data": function ( d ) {
							d.myKey = "myValue";
...
var csrfToken = <?= json_encode($this->request->getParam('_csrfToken')) ?>;
		jQuery(document).ready(function() {
			...
			jQuery("body").on("click", "#ammend_approve", function(event){
				...
				if(confirm('Are you Sure approve this P.O.?')) {
					var curr_data = {	 						 					
						po_id : po_id,	 					
					};	 				
					jQuery.ajax({
                ...
                type:"POST",
						url:"<?php echo Router::url(array('controller'=>'Ajaxfunction','action'=>'approveammendpo'));?>",
						data:curr_data,
						async:false,
...
var csrfToken = <?= json_encode($this->request->getParam('_csrfToken')) ?>;
	...
	// PO Delivery History Record AJAX
