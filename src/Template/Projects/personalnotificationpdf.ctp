<?php 
	...
	error_reporting(0);
		...
		header('Content-Disposition: inline; filename="personal_notification_list.pdf"');
		...
		$mpdf->WriteHTML('</head>');
		$mpdf->WriteHTML('<body>');	
		$mpdf->SetTitle('Personal Notification List');	
		...
		$mpdf->WriteHTML("<td align=left><b>Personal Notification List</b></td>");
		...
		unset($rows[0]);
		$header = array("Message","Event Date","Type Of Event");
