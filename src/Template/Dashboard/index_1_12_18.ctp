...
<div class="row" style="margin-top: 10px;">
				  ...
				  <table class="table dashboard_info_box">
						...
						<tbody>
							...
							<?php	}		else{ ?>
									...
									<td class='border-right'><?php  $pv = $this->ERPfunction->get_total_pricevariation($project_details[0]['project_id']); ?></td>
				  ...
				  <table class="table dashboard_info_box">
						<thead>
							...
							if(!empty($alerts))
							{
								...
								if($role == 'constructionmanager' || $role == 'billingengineer' || $role == 'pmm' || $role == 'pmm')
								{
									echo "<tr>
						...
						<table class="table dashboard_info_box">
							<?php
								echo "<tr>
									...
									<td><a href='{$this->request->base}/Purchase/viewporecords'>".$this->ERPfunction->dashboard_po_count($project_id,$date_from,$date_to)."<i class='icon-eye-open' style='padding-left:10px;font-size:20px' ></i> </a></td>
								...
								if(!empty($po_data))
								{
									...
									foreach($po_data as $po)
									{
										...
										echo "<tr>										
											...
											<td><span class='custom'>{$po['amount']}</span></td>
											</tr>";
									}
								}else{
									echo '<tr style="color:#000 !important">
						...
						<table class="table dashboard_info_box">
							...
							$count=count($this->ERPfunction->dashboard_wo_list($project_id,$date_from,$date_to));
								...
								echo "<tr>
									...
									<td><a href='{$this->request->base}/Contract/worecords'>".$count."<i class='icon-eye-open' style='padding-left:10px;font-size:20px' ></i> </a></td>
								...
								if(!empty($wo_data))
								{
									...
									foreach($wo_data as $wo)
									{
										...
										echo "<tr>										
											...
											<td><span class='custom'>{$wo['amount']}</span></td>
											</tr>";
										}
										$i++;
									}
	...
	if($role == 'erphead' || $role == 'ceo' || $role == 'md' || $role == 'erpoperator' || $role == 'erpmanager' || $role == 'projectdirector' || $role == 'constructionmanager' || $role == 'billingengineer' || $role == 'purchasehead' || $role == 'purchasemanager' || $role == 'siteaccountant' || $role == 'senioraccountant')
		{
		if(isset($project_id)){
			?>
		<div class="block block-drop-shadow dashboard col-xs-12 col-md-12 col-sm-12" id="boxes">
							...
							if(!empty($invetory_data))
							{
								foreach($invetory_data as $invetory)
								{
									...
									echo "<tr>										
										...
										<td '>{$invetory['total_in']}</td>
										<td>{$invetory['total_out']}</td>
										<td>{$this->ERPfunction->dashboard_inventory_stock($project_id,$invetory['material_id'],'stock_in')}</td>
										<td>{$this->ERPfunction->dashboard_inventory_stock($project_id,$invetory['material_id'],'stock_out')}</td>
										<td>{$this->ERPfunction->get_current_stock($project_id,$invetory['material_id'])}</td>
