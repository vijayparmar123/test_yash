		...
		jQuery('#user_list').DataTable({
			...
			<table id="user_list" class="dataTables_wrapper table table-striped table-hover" style="width:100%;">
				...
				array('class'=>'img-circle','height'=>'50px','width'=>'50px'));*/ ?>							
								...
								<td></td>								
								<td>
								<?php if($this->ERPfunction->retrive_accessrights($role,'personalEmployeeEdit')==1)
								{
								echo ' ';
								echo $this->Html->link("<i class='icon-pencil'></i> Edit",array('action' => 'personaleditemployee', $retrive_data['user_id']),
								array('class'=>'btn btn-primary btn-clean','escape'=>false,"target"=>"_blank"));
 								...
 								if($this->ERPfunction->retrive_accessrights($role,'personnel')==1)
								{							
								echo $this->Html->link("<i class='icon-eye-open'></i> View",array('action' => 'viewpersonnel', $retrive_data['user_id']),
