...
$allow_upload = ($this->ERPfunction->retrive_accessrights($role,'hrfileadd')==1)?true:false; // Set to true to allow upload files
...
<?php endif; ?>
	...
	function renderFileRow(data) {
		...
		var $dl_link = '';
		<?php if($this->ERPfunction->retrive_accessrights($role,'hrfiledownload')==1){?>
		var $dl_link = $('<a/>').attr('href','?do=download&file='+ encodeURIComponent(data.path))
			.addClass('download').text('download');
		<?php } ?>
		var $delete_link = '';
		<?php if($this->ERPfunction->retrive_accessrights($role,'hrfiledelete')==1){?>
...
$(function(){
	...
	if(hash != 'hr')
	{
		if(sub_location != '')
		{
			window.location.href = baseUrl+"#hr%2F"+sub_location;
			return false;
		}else{
			window.location.href = baseUrl+"#hr";
			return false;
		}
	}
	// else{
