				...
				<tbody>
					...
					{				
						...
						foreach($loan_data as $user)
						{
							...
							echo "<td>";
							if($this->ERPfunction->retrive_accessrights($role,'viewloan')==1 )
							{
								echo "<a href='{$this->request->base}/humanresource/viewloan/{$user['loan_id']}' class='btn btn-clean btn-info'><i class='icon-eye-open'></i>view</a>";
							}
							if($this->ERPfunction->retrive_accessrights($role,'payloanhistory')==1 )
							{
								echo "
