...
ob_clean();
	...
	// $mpdf->WriteHTML("<td><strong>Employee No :</strong> {$data["user_id"]}</td>");
	$mpdf->WriteHTML("<td><strong>Employee No :</strong> {$this->ERPfunction->get_user_pf_ref_no($data["user_id"])}</td>");
	...
	$mpdf->WriteHTML("<td><strong>Date of Joining :</strong> ".date("d-m-y",strtotime($this->ERPfunction->get_user_joindate($data["user_id"])))."</td>");
	$mpdf->WriteHTML("</tr>");
	...
	$mpdf->WriteHTML("<td><strong>Birth Date :</strong>".date("d-m-y",strtotime($this->ERPfunction->get_user_birthdate($data["user_id"])))."</td>");
	$mpdf->WriteHTML("</tr>");
	...
	$mpdf->WriteHTML("<td><strong>Total Days of Month :</strong> {$data["total_days"]}</td>");
	$mpdf->WriteHTML("</tr>");
	...
	<table width=100% class="count" style="background:#F2DBDB">
		<thead>
			<tr>
				<th colspan=3 style="background: #D99594;border-bottom:0">
					EARNINGS
				</th>
			</tr>
			<tr>
		...
		<tbody>
			<tr>
				<td>Basic Pay</td>
				<td align="center">'.$data["basic_pay_ctc"].'</td>
				<td align="center" >'.$data["basic_pay_amount"].'</td>
			</tr>
			<tr>
				<td>Dearness Allowance (D.A.)</td>
				<td align="center">'.$data["da_ctc"].'</td>
			...
			<tr>
				...
				($acco_text == 1)
					?
						$data["acco_ctc"] .
						" <span style='background:#D99594;'> Bill Paid&nbsp;</label>"
					:									
						"<span style='font-size:10px;'>".strtoupper(str_replace("Fixed","",str_replace("_"," ",$data["acco_allowance"])))."</span>"
				)
				.'</td>								
			...
			<tr>
				...
				<td align="center">'.$data["hra_ctc"].'</td>
				<td align="center" >'.$data["hra_amount"].'</td>
			</tr>
			<tr>
				<td>Travel Allowance (T.A.)</td>
				<td align="center">
				'.
				(
				/*$data["transport_ctc"]*/
				($trans_text == 1)
					?
						...
						" <span style='background:#D99594;'> Bill Paid&nbsp;</label>"
					:									
						"<span style='font-size:10px;'>".strtoupper(str_replace("Fixed","",str_replace("_"," ",$data["trans_allowance"])))."</span>"
				)
				.'</td>					
			...
			<tr>
				...
				<td align="center">'.$data["medical_ctc"].'</td>
			...
			<tr>
				...
				(
					...
					?
						...
						" <span style='background:#D99594;'> Bill Paid&nbsp;</label>"
					:									
						"<span style='font-size:10px;'>".strtoupper(str_replace("Fixed","",str_replace("_"," ",$data["food_allowance"])))."</span>"			
				).'
				</td>								
			...
			<tr>
				...
				($mobile_text == 1)
					?
						...
						" <span style='background:#D99594;'> Bill Paid&nbsp;</label>"
					:									
						"<span style='font-size:10px;'>".strtoupper(str_replace("Fixed","",str_replace("_"," ",$data["mobile_allowance"])))."</span>"
				)				
				.'</td>								
			...
			<tfoot>
			<tr>
				<td colspan=2 style="background: #D99594;border-bottom:0">					
