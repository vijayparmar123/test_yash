		...
		jQuery('#user_list').DataTable({
			...
			<table id="user_list" class="dataTables_wrapper table table-striped table-hover" style="width:100%;">
				...
				array('class'=>'img-circle','height'=>'50px','width'=>'50px'));*/ ?>							
								...
								<td>
								<?php
 								if($this->ERPfunction->retrive_accessrights($role,'notworkingemplyeelist')==1)
								{								
								...
								if($this->ERPfunction->retrive_accessrights($role,'Rejoin')==1 && $retrive_data['already_rejoined'] == 0)
								{	
									echo' ';
									echo $this->Html->link("<i class='icon-pencil'></i> Rejoin",array('action' => 'rejoin', $retrive_data['user_id']),
