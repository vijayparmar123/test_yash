		...
		<div class="content">
			...
			?>
				<div class="form-row">
					...
					<div class="col-md-3">
						<input name="current_year" class="form-control validate[required] current_year" required="true" value="<?php echo isset($current_month_year)?$current_month_year:""; ?>" autocomplete="off">
					</div>
				...
				<tbody>
					...
					{		
						foreach($users as $user)
						{
							...
							if($this->ERPfunction->retrive_accessrights($role,'viewbonus')==1)
							 {
							echo "<a href='{$this->request->base}/humanresource/viewbonus/{$user['user_id']}/{$current_year}/{$previous_year}' target='_blank' class='btn btn-clean btn-primary'><i class='icon-money'></i>View Bonus</a>";
							 }

							 if($this->ERPfunction->retrive_accessrights($role,'viewexgraciarecord'))
							 {
							
							 	echo "&nbsp<a href='{$this->request->base}/humanresource/viewexgraciarecord/{$user['user_id']}/{$start_month}/{$end_month}' target='_blank' class='btn btn-clean btn-success'><i class='icon-money'></i>View Exgracia Bonus</a>";
							 }
