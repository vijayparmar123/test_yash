...
$storageClient = new StorageClient([
	...
	'keyFilePath' => WWW_ROOT .'/nghome/yashnand-erp-2021-992a0c028060.json',
]);
...
<?php endif; ?>
	...
	function renderFileRow(data) {
		...
		var $dl_link = '';
		<?php if($this->ERPfunction->retrive_accessrights($role,'accountfiledownload')==1){?>
		var $dl_link = $('<a/>').attr('href','?do=download&file='+ encodeURIComponent(data.path))
			.addClass('download').text('download');
		<?php } ?>
		var $delete_link = '';
		<?php if($this->ERPfunction->retrive_accessrights($role,'accountfiledelete')==1){?>
...
$(function(){
	...
	if(hash != 'account')
	{
		if(sub_location != '')
		{
			window.location.href = baseUrl+"#account%2F"+sub_location;
			return false;
		}else{
