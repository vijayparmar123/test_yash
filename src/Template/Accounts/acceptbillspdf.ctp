<?php 
	...
	error_reporting(0);
		...
		header('Content-Disposition: inline; filename="acceptbills.pdf"');
		...
		$mpdf->WriteHTML('</head>');
		$mpdf->WriteHTML('<body>');	
		$mpdf->SetTitle('Accept Bills List');	
		...
		$mpdf->WriteHTML("<td align=left><b>Accept Bills LIST</b></td>");
		...
		unset($rows[0]);
		$header = array("Inward Date","Project Name","Bill Inward No","Party Name","Type of Bill","Invoice No","Total Amount","Bill Date","Credit Period","Diff(+/-)","Qty. Checked By","Rate Checked By");
