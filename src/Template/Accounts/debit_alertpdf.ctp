<?php 
	...
	error_reporting(0);
		...
		header('Content-Disposition: inline; filename="debitalert_list.pdf"');
		...
		$mpdf->WriteHTML('</head>');
		$mpdf->WriteHTML('<body>');	
		$mpdf->SetTitle('DEBIT ALERT LIST');	
		...
		$mpdf->WriteHTML("<td align=left><b>DEBIT ALERT LIST</b></td>");
		...
		unset($rows[0]);
		$header = array("Debit Note No","Date","Receiver Party Name","Given To","Total Amount of Debit(Rs)","Attachment");
