			...
			<table id="request_list"  class="dataTables_wrapper table table-striped table-hover">
				<thead>
					<tr>
						...
						<?php
						if($this->ERPfunction->retrive_accessrights($role,'debitnotealert')==1)
								{
						?>
						<th>Edit / Delete</th>
						<?php
						}
						 if($this->ERPfunction->retrive_accessrights($role,'approve_debit_cm_md_pd')==1)
						{
						?>
						...
						?>
						<?php
						 if($this->ERPfunction->retrive_accessrights($role,'approve_debit_sr_ac')==1)
						{
						?>
					...
					<?php
						...
						?>
							<tr>
                                ...
                                <td><?php 
								if($this->ERPfunction->retrive_accessrights($role,'editdebit')==1)
								{
								echo $this->Html->link("<i class='icon-pencil'></i> Edit",array('action' => 'editdebit', $debit['debit_id']),
								...
								if($this->ERPfunction->retrive_accessrights($role,'deletedebit')==1)
								{
								...
								<?php
								$approval_cmpdmd = $this->ERPfunction->check_debit_approve_first_step($debit['debit_id']);
								
								if($this->ERPfunction->retrive_accessrights($role,'approve_debit_cm_md_pd')==1)
								{
								
								?>
                               <td align="center"><input type="checkbox" name="cmmdpd[]" id="cmmdpd" class="cmpdmd" value=""  data_id="<?php echo $approval_cmpdmd == 1 ? '' : $debit['debit_id']; ?>" <?php echo ($approval_cmpdmd==1 ? 'checked' : '');?> <?php echo ($approval_cmpdmd==1 ? 'DISABLED' : '');?>></td>
								<?php
									 }
								?>
								...
								<?php
								 if($this->ERPfunction->retrive_accessrights($role,'approve_debit_sr_ac')==1)
								{
								$approval_acc = $this->ERPfunction->check_debit_approve_second_step($debit['debit_id']);
								?>
								<td align="center"><input type="checkbox" name="account[]" id="account" class="account" value=""  data_id="<?php echo $approval_acc == 1 ? '' : $debit['debit_id']; ?>" <?php echo ($approval_acc==1 ? 'checked' : '');?> <?php echo ($approval_acc==1 ? 'DISABLED' : '');?> <?php echo ($approval_cmpdmd==1 ? '' : 'DISABLED');?>></td>
								<?php 
								 }
								?>
							</tr>
