<?php 
	...
	error_reporting(0);
		...
		header('Content-Disposition: inline; filename="expencealert_list.pdf"');
		...
		$mpdf->WriteHTML('</head>');
		$mpdf->WriteHTML('<body>');	
		$mpdf->SetTitle('EXPENCE ALERT LIST');	
		...
		$mpdf->WriteHTML("<td align=left><b>EXPENCE ALERT LIST</b></td>");
		...
		unset($rows[0]);
		$header = array("Voucher No","Date","Expense Head","Given To","Payment","Total Amount of Expense");
