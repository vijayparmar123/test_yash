<?php
	...
	$mpdf->WriteHTML("<td>Date : {$data['el_date']->format('d-m-Y')}</td>");	
	$mpdf->WriteHTML("</tr>");
	...
	$mpdf->WriteHTML("<tr>");	
	$mpdf->WriteHTML("<td style='height:30px' >Asset Group : {$this->ERPfunction->get_asset_group_name($data['asset_group'])}</td>");
