<?php 
	...
	error_reporting(0);
		...
		header('Content-Disposition: inline; filename="assetTheftHistory.pdf"');
		...
		$mpdf->WriteHTML('</head>');
		$mpdf->WriteHTML('<body>');	
		$mpdf->SetTitle('Asset Theft History');	
		...
		$mpdf->WriteHTML("<td align=left><b>Asset Theft History</b></td>");
		...
		unset($rows[0]);
		$header = array("Theft from Project","Theft Date","Created By");
