...
jQuery(document).ready(function() {
	   ...
	   var curr_data = {	 						 					
	 	 ...
	 	 jQuery.ajax({
                ...
                success: function(response){					
					...
					jQuery('#asset_id').html(json_obj['asset_list']);	
					jQuery('#asset_id option[value=<?php echo $asset_name;?>]').attr("selected","selected");
					jQuery('#asset_id').select2();
					return false;
                },
                error: function (e) {
                     alert('Error');
	...
	jQuery("body").on("click","#editpo",function(){
			...
			if(confirm("Are you sure,you want to Edit Asset PO?"))
			{
				if(confirm("Are you sure,you want to Edit Asset PO?"))
				{
					return true;
	...
	jQuery("body").on("change", "#usage", function(){
		...
		if(usage_name == 'for_asset')
		{
			jQuery("#asset_div").show();
			jQuery("#asset_id").attr('required', true);
		}
		else if(usage_name == 'for_self')
		{
			jQuery("#asset_div").hide();
			jQuery("#asset_id").attr('required', false);
		}
   });
	  ...
	  var project_id  = jQuery(this).val() ;
	   var curr_data = {	 						 					
	 	 ...
	 	 jQuery.ajax({
                ...
                type:"POST",
                url:"<?php echo Router::url(array('controller'=>'Ajaxfunction','action'=>'inassetpoprojectdetail'));?>",
                data:curr_data,
                async:false,
                success: function(response){					
					var json_obj = jQuery.parseJSON(response);					
		...
		jQuery.ajax({
			headers: {
				'X-CSRF-Token': csrfToken
                     ...
                     type:'POST',
                     url: '<?php echo Router::url(["controller" => "Ajaxfunction","action" => "addnewrowpoasset"]);?>',
  ...
  jQuery('body').on('click','.del_parent',function(){
		...
		if(detail_id)
		{
	 	 ...
	 	 jQuery.ajax({
                ...
                type:"POST",
                url:"<?php echo Router::url(array('controller'=>'Ajaxfunction','action'=>'deleteassetpodetail'));?>",
                data:curr_data,
                async:false,
                success: function(response){
                ...
                <div class="block block-fill-white">
					 ...
					 <div class="content controls">
						...
						<div class="form-row">
                            ...
                            <div class="col-md-10">
								...
								<div class="radiobox-inline" style="padding:0 50px;">
                                    <label><input type="radio" name="bill_mode" value="haryana" class="bill_mode" <?php echo ($erp_po_details['bill_mode'] == 'haryana')?'checked':''; ?> />Haryana</label>
                                </div>
                            </div>
						...
						<div class="form-row">
                            ...
                            <div class="col-md-4">
								<select class="select2"  required="true" name="usage_name" style="width:100%;" id="usage">
									<option value="for_asset" <?php echo ($erp_po_details['usage_name'] == 'for_asset')?'selected':''; ?>>Existing Asset</Option>
									<option value="for_self" <?php echo ($erp_po_details['usage_name'] == 'for_self')?'selected':''; ?> >New Asset</Option>
								</select>
							</div>
							
							<div id="asset_div" style="display:<?php echo ($erp_po_details['usage_name'] == 'for_asset')?'':'none'; ?>"> 
							<div class="col-md-2">Asset:</div>
                            <div class="col-md-4">
								<select class="select2 asset_id" style="width: 100%;" name="asset_id" id="asset_id">
						...
						<div class="form-row">
                            ...
                            <div class="col-md-4">							
								<input type="text" name="po_no" id="po_no" value="<?php echo $erp_po_details["po_no"];?>" readonly="true" class="form-control" value=""/>
							</div>
					...
					<!--	<div class="form-row" id="radiogroup">                           
						...
						<div class="form-row" style="padding:15px 0 0 0;">
							...
							<p id="mp_address" <?php echo ($bill_mode == "mp")?'':'style="display:none;"';?>><strong>Billing Address:</strong><?php echo $this->ERPfunction->getmpbilladdress($erp_po_details["po_date"]); ?></p>
							...
							<p><strong>PAN No.:</strong><span id="pan_no"><?php echo $this->ERPfunction->getstatepanno($bill_mode,$erp_po_details["po_date"]); ?></span> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
							...
							<p>9) <?php echo $this->ERPfunction->getconditionofpowo($erp_po_details["po_date"],$bill_mode); ?></p>
