<?php 
	...
	error_reporting(0);
		...
		header('Content-Disposition: inline; filename="EquipmentLogOwn.pdf"');
		...
		$mpdf->WriteHTML('</head>');
		$mpdf->WriteHTML('<body>');	
		$mpdf->SetTitle('Equipment Log Records - Own');	
		...
		$mpdf->WriteHTML("<td align=left><b>Equipment Log Records - Own</b></td>");
		...
		unset($rows[0]);
		$header = array("Date","E.L No.","Asset Name","Identity/Vehi.No.","Operational Status","Usage(km)","Usage(hr.)","Driver Name");
