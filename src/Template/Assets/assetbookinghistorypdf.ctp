<?php 
	...
	error_reporting(0);
		...
		header('Content-Disposition: inline; filename="assetBookingHistory.pdf"');
		...
		$mpdf->WriteHTML('</head>');
		$mpdf->WriteHTML('<body>');	
		$mpdf->SetTitle('Asset Booking History');	
		...
		$mpdf->WriteHTML("<td align=left><b>Asset Booking History</b></td>");
		...
		unset($rows[0]);
		$header = array("Project","Date of Requirement","Date of Entry","Created By");
