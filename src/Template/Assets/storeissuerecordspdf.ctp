<?php 
	...
	error_reporting(0);
		...
		header('Content-Disposition: inline; filename="storeIssueRecords.pdf"');
		...
		$mpdf->WriteHTML('</head>');
		$mpdf->WriteHTML('<body>');	
		$mpdf->SetTitle('Store Issue Records');	
		...
		$mpdf->WriteHTML("<td align=left><b>Store Issue Records</b></td>");
		...
		unset($rows[0]);
		$header = array("Date of Issue","Asset ID","Asset Name","Capacity","Asset Make","Material Name","Quantity Issued","Unit");
