...
<div class="block block-fill-white">				
	...
	<div class="content controls">
		<div class="form-row">
			...
			<div class="col-md-4">
				<select style="width: 100%;" class="select2" disabled required="true"  name="asset_name" id="asset_namelist">
					<option value="">Select Assets</option>
					<?php 
					foreach($asset_names as $key => $retrive_data)
					{
						$selected = ($retrive_data['asset_id']==$data['asset_id'])?"selected":"";
			...
			<div class="col-md-4">
				<input type="text" readonly="true" id="asset_code" name="asset_code" value="<?php echo isset($data)?$data['asset_code']:""; ?>" class="form-control"/>
			</div>
		</div>
		<div class="form-row">
			<div class="col-md-2">Make</div>
			<div class="col-md-4">
				<input type="text" id="asset_make" readonly="true" name="asset_make" value="<?php echo isset($data)?$data['asset_make']:""; ?>" class="form-control"/>	
			</div>
			
			<div class="col-md-2">Asset Capacity</div>
			<div class="col-md-4">
				<input type="text" readonly="true" id="capacity" name="capacity" value="<?php echo isset($data)?$data['asset_capacity']:""; ?>" class="form-control"/>
			</div>
		</div>
		...
		<div class="form-row">
			...
			<div class="col-md-4"><input type="text" readonly="true" name="model_no" id="model_no" value="<?php echo isset($data)?$data['model_no']:""; ?>" class="form-control validate[required]"/></div>
			<div class="col-md-2">Identity / Veh. No.</div>
			<div class="col-md-4"><input type="text" readonly="true" name="vehicle_no" id="vehicle_no" value="<?php echo isset($data)?$data['identity']:""; ?>" class="form-control"/></div>
		</div>	
			...
			<input type="hidden" class="form-control deploy_to_project" name="deploy_to_project" value="">
			 <select style="width: 100%;" class="select2" disabled="disabled" readonly="true" required="true"  name="deployed_to" id="deployed_to">
				...
				foreach($project_data as $key => $retrive_data)
				{
					...
					echo '<option value="'.$retrive_data['project_id'].'" '.$this->ERPfunction->selected($retrive_data['project_id'],$data['deploy_to']).'>'.$this->ERPfunction->get_projectname($retrive_data['project_id']).'</option>';
				}  
				?>
				</select> 
			</div>
		...
		<div class="form-row">
			...
			<div class="col-md-10"><textarea name="messages" readonly="true" class="validate[required] form-control"><?php echo isset($data)?$data['message']:""; ?></textarea> </div>
		</div>
		...
		<div class="form-row">
			...
			<div class="col-md-3">
				<select style="width: 100%;" class="select2" disabled required="true"  name="event_type" id="event_type">
