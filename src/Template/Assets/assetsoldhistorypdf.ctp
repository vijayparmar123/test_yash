<?php 
	...
	error_reporting(0);
		...
		header('Content-Disposition: inline; filename="assetSoldHistory.pdf"');
		...
		$mpdf->WriteHTML('</head>');
		$mpdf->WriteHTML('<body>');	
		$mpdf->SetTitle('Asset Sold History');	
		...
		$mpdf->WriteHTML("<td align=left><b>Asset Sold History</b></td>");
		...
		unset($rows[0]);
		$header = array("Sold from Project","Sold Date","Created By");
