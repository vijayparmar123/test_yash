<?php 
	...
	error_reporting(0);
		...
		header('Content-Disposition: inline; filename="assetTransferHistory.pdf"');
		...
		$mpdf->WriteHTML('</head>');
		$mpdf->WriteHTML('<body>');	
		$mpdf->SetTitle('Asset Transfer History');	
		...
		$mpdf->WriteHTML("<td align=left><b>Asset Transfer History</b></td>");
		...
		unset($rows[0]);
		$header = array("Transferred Date","Transfer From","Transfer To","Transfer by","Accept Date","Accepted By");
