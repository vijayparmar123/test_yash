<?php 
	...
	error_reporting(0);
		...
		header('Content-Disposition: inline; filename="assetIssueHistory.pdf"');
		...
		$mpdf->WriteHTML('</head>');
		$mpdf->WriteHTML('<body>');	
		$mpdf->SetTitle('Asset Issued History');	
		...
		$mpdf->WriteHTML("<td align=left><b>Asset Issued History</b></td>");
		...
		unset($rows[0]);
		$header = array("Issued To","Date of Issue","Date of Return","Project","Created By");
