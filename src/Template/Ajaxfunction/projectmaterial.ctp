...
jQuery(document).ready(function() {
	...
	jQuery("body").on("click", "#save", function(event){	 
	  ...
	  var brand_title  = jQuery("#brand_title").val();
	  var project_id  = jQuery("#project_id").val();
	  var unit  = jQuery("#unit").val();
	  var consume  = jQuery("#consume").val();
	  var cost_group  = jQuery("#cost_group").val();
	  if(material_code == '' || material_item_code == '' || material_title == '' || unit == '' || consume == '' || project_id == '' || cost_group == '' || brand_title == '')
	  {
	  ...
	  /* jQuery("#brand_id").html(""); */
		...
		return false; */
	   var curr_data = {
	 					material_code : material_code, material_item_code : material_item_code , material_title :material_title , project_id : project_id , unit : unit, consume : consume, cost_group : cost_group	, brand_title :brand_title			
	 					};	 				
	 	 jQuery.ajax({
                headers: {
				'X-CSRF-Token': csrfToken
                ...
                type:"POST",
                url:"<?php echo Router::url(array('controller'=>'Ajaxfunction','action'=>'addprojectmaterial'));?>",
                data:curr_data,
                async:false,
                success: function(response){
	...
	//actual
	 ...
	 // jQuery("body").on("click", "#save", function(event){	 
	  ...
	  // if(material_code == '' || material_item_code == '' || material_title == '' || unit == '' || brand_title == '' || consume == '')
	  // {
		  // alert('Please fill all field');
		  // return false;
	  // }
	   ...
	   // var curr_data = {
	 					// material_code : material_code, material_item_code : material_item_code , material_title :material_title , brand_title :brand_title , project_id : project_id , unit : unit , consume : consume, cost_group : cost_group					
	 					// };
...
<form id="material_form" method="post" class="form-horizontal transferform">
						...
						<div class="form-row">
                            ...
                            <div class="col-md-8">
								...
								<input type="hidden" name="material_code" value="17" id="material_code"/>
								<input type="hidden" name="material_item_code" value="YNEC/MT/TMP/17" id="material_item_code"/>
							</div>
                        </div>
						
						<div class="form-row">
							<div class="col-md-4">Brand Title<span class="require-field">*</span></div>
                            <div class="col-md-8">
								<input type="text" name="brand_title" id="brand_title" class="form-control validate[required]"/>
							</div>
                        </div>
