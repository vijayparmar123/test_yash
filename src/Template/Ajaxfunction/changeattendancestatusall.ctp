...
<div class="controls">
	...
	<!-- <form name="medicinecat_form" action="<?php echo $this->request->base;?>/attendance/attendancealert" method="post" class="form-horizontal transferform"> -->
		<?php 
				echo $this->Form->Create('medicinecat_form',['class'=>'form_horizontal transferform','method'=>'post','url' => ['controller' => 'attendance','action' => 'attendancealert']]);
			?>
		<div class="form-row">
			...
			<div class="col-sm-4">
				...
				<input type="hidden" name="emp_at" value='<?php echo $emp_at;?>'>
				<input type="hidden" name="date" value='<?php echo $day."-".$month."-".$year;?>'>
				<input type="submit" value="Update" name="edit_all" class="btn btn-primary" id="btn-add-category"/>
			</div>
		</div>
	<?php 
		echo $this->Form->end();
