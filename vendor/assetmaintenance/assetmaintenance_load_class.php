...
class SSP_maintenance {
	...
	static function data_output ( $columns, $data ,$user)
	{
		...
		for ( $i=0, $ien=count($data) ; $i<$ien ; $i++ ) {
			...
			for ( $j=0, $jen=count($columns) ; $j<$jen ; $j++ ) {
				...
				{
					$uid=$data[$i][ $columns[$j]['field'] ];
					$maintenance_type = ($uid)?"Corrective / Breakdown":"Preventive / Routine";
					 $row[ $column['dt'] ] = $maintenance_type;
				}
				else if($j== 11)
				{
					$uid=$data[$i][ $columns[$j]['field'] ];
					$payment_by = ($uid == 1)?"Cash":"Cheque";
					 $row[ $column['dt'] ] = $payment_by;
				}
				else if($j== 12)
				{
					$uid=$data[$i][ $columns[$j]['field'] ];
					...
					$html .= "<a href='viewaddmaintenance/{$uid}' target='_blank' class='btn btn-primary btn-clean'><i class='icon-pencil'></i> View</a>";
					
					if(self::retrive_accessrights($role,'unapprovemaintenance')==1)
					{
						$html .= "<a href='unapprovemaintenance/{$uid}' class='btn btn-danger btn-clean cancelmaintenance'><i class='icon-pencil'></i> Delete</a>";
					}
	...
	{	
		...
		if(!empty($post))
		{
		$or["maintenance.project_id IN"] = (!empty($post["pro_id"]) && $post["pro_id"][0] != "All" )?$post["pro_id"]:NULL;
		$or["erp_assets.asset_id IN"] = (!empty($post["asset_name"]) && $post["asset_name"][0] != "All" )?$post["asset_name"]:NULL;
		$or["maintenance.asset_group IN"] = (!empty($post["asset_group"]) && $post["asset_group"][0] != "All")?$post["asset_group"]:NULL;
		$or["maintenance.maintenance_type IN"] = (!empty($post["maintenance_type"]) && $post["maintenance_type"][0] != "All")?$post["maintenance_type"]:NULL;
		$or["maintenance.payment_by IN"] = (!empty($post["payment_type"]) && $post["payment_type"][0] != "All")?$post["payment_type"]:NULL;
		$or["maintenance.vehicle_no ="] = (!empty($post["identity"]) && $post["identity"][0] != "All" )?$post["identity"]:NULL;
		
		if($or["maintenance.project_id IN"] == NULL)
		{
			if(self::project_alloted($role)==1)
			{ 
				$or["maintenance.project_id IN"] = implode(",",$projects_ids);
			}
		}
		...
		$or["maintenance.approved_status ="] = 1;
		// echo "<pre>";
		// print_r($post);
		// echo "</pre>";
		// echo "<pre>";
		...
		foreach($or as $key=>$value)
		{
			if($i == 0)
			{
				...
				}elseif($key == "erp_assets.asset_id IN")
				{
					$extraquery .= ' '. $key .' '. '('.$value.')';
				}elseif($key == "maintenance.asset_group IN")
				{
					$extraquery .= ' '. $key .' '. '('.$value.')';
				}elseif($key == "maintenance.maintenance_type IN")
				{
					$extraquery .= ' '. $key .' '. '('.$value.')';
				}elseif($key == "maintenance.payment_by IN")
				{
					$extraquery .= ' '. $key .' '. '('.$value.')';
				}else
				{
