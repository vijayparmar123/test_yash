...
class SSP_mrn {
		...
		for ( $i=0, $ien=count($data) ; $i<$ien ; $i++ ) {
			...
			for ( $j=0, $jen=count($columns) ; $j<$jen ; $j++ ) {
				...
				else if($j== 10)
				{
					...
					if(self::retrive_accessrights($role,'unapprovemrn')==1)
					{
						$html .= "<a href='unapprovemrn/{$detail_id}' class='btn btn-danger btn-clean'><i class='icon-pencil'></i> Delete</a>";
					}
					if(self::retrive_accessrights($role,'previewapprovedmrn')==1)
					{
						$html .= "<a href='previewapprovedmrn/{$uid}' target='_blank' class='btn btn-primary btn-clean'><i class='icon-eye-open'></i> View</a>";
					}
					$row[ $column['dt'] ] =$html;
				}
				else {
	...
	{	
		...
		$or["mrn.project_id IN"] = (!empty($post["pro_id"]) && $post["pro_id"] != "All" )?$post["pro_id"]:NULL;
		$or["mrn_detail.material_id IN"] = (!empty($post["material_id"]) && $post["material_id"] != "All")?$post["material_id"]:NULL;
		$or["mrn.vendor_userid IN"] = (!empty($post["vendor_userid"]) && $post["vendor_userid"] != "All")?$post["vendor_userid"]:NULL;
		$or["mrn.mrn_date >="] = ($post["date_from"] != "")?date("Y-m-d",strtotime($post["date_from"])):NULL;
		$or["mrn.mrn_date <="] = ($post["date_to"] != "")?date("Y-m-d",strtotime($post["date_to"])):NULL;
		$or["mrn.mrn_no ="] = (!empty($post["mrn_no"]))?$post["mrn_no"]:NULL;
		
		if($or["mrn.project_id IN"] == NULL)
		{
			if(self::project_alloted($role)==1)
			{ 
				$or["mrn.project_id IN"] = implode(",",$projects_ids);
			}
		}
		...
		foreach($or as $key=>$value)
		{
			if($i == 0)
			{
				if($key == "mrn.project_id IN")
				{
					$extraquery .= ' '. $key .' '. '('.$value.')';
					// $extraquery .= ' '. $key .' '. '('.implode(",",$value).')';
				}elseif($key == "mrn_detail.material_id IN")
				{
					$extraquery .= ' '. $key .' '. '('.$value.')';
				}elseif($key == "mrn.vendor_userid IN")
				{
					$extraquery .= ' '. $key .' '. '('.$value.')';
				}else
				{
