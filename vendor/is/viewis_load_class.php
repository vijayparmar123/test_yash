...
class SSP_is {
	...
	static function data_output ( $columns, $data ,$user)
	{
		...
		for ( $i=0, $ien=count($data) ; $i<$ien ; $i++ ) {
			...
			for ( $j=0, $jen=count($columns) ; $j<$jen ; $j++ ) {
				...
				{
					$uid=$data[$i][ $columns[$j]['field'] ];
					$is_asset = explode("_",$uid);
					if(isset($is_asset[1]))
					{
						$agency = self::get_asset_name($is_asset[1]);
					}else{
						$agency = self::get_vendor_name($uid);
					}
					$row[ $column['dt'] ] = $agency;
				}
				else if($j== 4)
				{
					$uid=$data[$i][ $columns[$j]['field'] ];
				...
				else if($j== 7)
				{
					...
					$is_id = $data[$i][ $columns[9]['field'] ];
					$details = self::get_approveis_details($is_id);
					$unit = self::get_items_units($material_id);
					$row[ $column['dt'] ] = $unit;
				}else if($j== 9)
				{
					$uid=$data[$i][ $columns[$j]['field'] ];
					$is_detail_id=$data[$i][ $columns[10]['field'] ];
					$changes_status=$data[$i][ $columns[11]['field'] ];
					$role = self::get_user_role($user);
					$html = '';
					
					if(self::retrive_accessrights($role,'previewapprovedis')==1)
					{
						$html .= "<a href='previewapprovedis/{$uid}' target='_blank' class='btn btn-primary btn-clean'><i class='icon-eye-open'></i> View</a>";
					}
					
					if(self::retrive_accessrights($role,'updateis')==1)
					{
						$html .= "<a href='updateis/{$uid}' target='_blank' class='btn btn-primary btn-clean'><i class='icon-edit'></i> Edit</a>";
					}
					
					if(self::retrive_accessrights($role,'unapproveis')==1)
					{
						$html .= "<a href='unapproveis/{$is_detail_id}' target='_blank' class='btn btn-danger btn-clean'><i class='icon-trash'></i> Delete</a>";
					}
					if($changes_status == 1)
					{
						if(self::retrive_accessrights($role,'auditischanges')==1)
						{
							$html .= "<a href='auditischanges/{$uid}' target='_blank' class='btn btn-primary btn-clean'><i class='icon-pencil'></i> Changes</a>";
						}
	...
	{	
		...
		if(!empty($post))
		{	
			$or["erp_is.project_id IN"] = (!empty($post["pro_id"]) && $post["pro_id"] != "All" )?$post["pro_id"]:NULL;
			$or["is_detail.material_id IN"] = (!empty($post["material_id"]) && $post["material_id"] != "All")?$post["material_id"]:NULL;
			$or["erp_is.agency_name IN"] = (!empty($post["agency_id"]) && $post["agency_id"] != "All")?$post["agency_id"]:NULL;
			$or["erp_is.is_date >="] = ($post["date_from"] != "")?date("Y-m-d",strtotime($post["date_from"])):NULL;
			$or["erp_is.is_date <="] = ($post["date_to"] != "")?date("Y-m-d",strtotime($post["date_to"])):NULL;
			$or["erp_is.is_no ="] = (!empty($post["is_no"]))?$post["is_no"]:NULL;
	...
	static function get_agency_name_by_code($code)
	{
		...
		$agency = $ag_tbl->find()->where(["vendor_id"=>$code])->hydrate(false)->toArray();
		if(!empty($agency))
		{
	...
	static function get_approveis_details($is_id)
	{
	...
	static function get_vendor_name($id)
	{
		$vendor_tbl = TableRegistry::get("erp_vendor");
		$vendor = $vendor_tbl->find()->where(["user_id"=>$id])->hydrate(false)->toArray();
		if(!empty($vendor))
		{
