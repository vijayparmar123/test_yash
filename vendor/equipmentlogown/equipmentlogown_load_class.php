...
class SSP_equipmentlogown {
		...
		for ( $i=0, $ien=count($data) ; $i<$ien ; $i++ ) {
			...
			for ( $j=0, $jen=count($columns) ; $j<$jen ; $j++ ) {
				...
				else if($j== 9)
				{
					...
					$html .= "<a href='viewaddequipmentown/{$uid}' target='_blank' class='btn btn-primary btn-clean'><i class='icon-eye-open'></i> View</a>";
					
					if(self::retrive_accessrights($role,'editequipmentlogowned')==1)
					{
						$html .= "<a href='editequipmentlogowned/{$uid}' target='_blank' class='btn btn-primary btn-clean'><i class='icon-pencil'></i> Edit</a>";
					}
					if(self::retrive_accessrights($role,'deleteequipmentlogown')==1)
					{
						$html .= "<a href='deleteequipmentlogown/{$uid}' class='btn btn-danger btn-clean deleteequipmentlog'><i class='icon-trash'></i> Delete</a>";
					}
	...
	{	
		...
		if(!empty($post))
		{
		$or["equipment.date >="] = ($post["date_from"] != "")?date("Y-m-d",strtotime($post["date_from"])):NULL;
		$or["equipment.date <="] = ($post["date_to"] != "")?date("Y-m-d",strtotime($post["date_to"])):NULL;
		$or["equipment.el_no ="] = (!empty($post["elno"]))?$post["elno"]:NULL;
		$or["equipment.project_id IN"] = (!empty($post["project_id"]) && $post["project_id"][0] != "All" )?$post["project_id"]:NULL;
		$or["erp_assets.asset_id IN"] = (!empty($post["asset_name"]) && $post["asset_name"][0] != "All" )?$post["asset_name"]:NULL;
		$or["erp_assets.asset_code ="] = (!empty($post["asset_id"]))?$post["asset_id"]:NULL;
		$or["equipment.asset_identity ="] = (!empty($post["vehicle_no"]))?$post["vehicle_no"]:NULL;
		$or["equipment.ownership ="] = (!empty($post["ownership"]) && $post["ownership"] != "All")?$post["ownership"]:NULL;
		
		if($or["equipment.project_id IN"] == NULL)
		{
			if(self::project_alloted($role)==1)
			{ 
				$or["equipment.project_id IN"] = implode(",",$projects_ids);
			}
		}
		...
		foreach($or as $key=>$value)
		{
			if($i == 0)
			{
				if($key == "equipment.project_id IN")
				{
