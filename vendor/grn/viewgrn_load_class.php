...
class SSP_grn {
		...
		for ( $i=0, $ien=count($data) ; $i<$ien ; $i++ ) {
			...
			for ( $j=0, $jen=count($columns) ; $j<$jen ; $j++ ) {
				...
				else if($j== 6)
				{
					...
					if($uid)
					{
						$material = $data[$i][ $columns[14]['field'] ];
					}else{
						$material = $data[$i][ $columns[16]['field'] ];
					}
				...
				else if($j== 7)
				{
					...
					if($uid)
					{
						$brand = $data[$i][ $columns[15]['field'] ];
					}else{
						// $brand = $data[$i][ $columns[22]['field'] ];
						$brand = self::get_brandname($data[$i][ $columns[18]['field'] ]);
					}
					$row[ $column['dt'] ] = $brand ;
				}
				else if($j== 8)
				...
				// {
					...
					// if($uid)
					// {
						// $code = $data[$i][ $columns[19]['field'] ];
					// }else{
						// $code = $data[$i][ $columns[12]['field'] ];
						// $code = self::get_items_units($data[$i][ $columns[23]['field'] ]);
					// }
					// $row[ $column['dt'] ] = $code ;
				// }
				// else if($j== 12)
				// {
					// $uid=$data[$i][ $columns[$j]['field'] ];
					// $type = ($uid != "")?'Central':'Local';
					 // $row[ $column['dt'] ] = $type;
				// }
				else if($j== 12)
				{
					// $uid=$data[$i][ $columns[$j]['field'] ];
					// $attach_label=$data[$i][ $columns[16]['field'] ];
					// $attached_files = json_decode($uid);	
					// $attached_label = json_decode(stripcslashes($attach_label));	
					// $html = "";
					...
					// {							
						...
						// { 
						   // if(!empty($file))
						   // {
								// $html .= "<a href='../webroot/img/users_images/{$file}' download='{$attached_label[$i]}' class='btn btn-info btn-clean'><i class='icon-download-alt'></i>{$attached_label[$i]}</a>";
							 // $i++;
							// }
						// }
					// }
				...
				{
					$uid=$data[$i][ $columns[$j]['field'] ];
					$detail_id=$data[$i][ $columns[21]['field'] ];
					$changes_status=$data[$i][ $columns[22]['field'] ];
					$role = self::get_user_role($user);
					$html = '';
					
					if(self::retrive_accessrights($role,'deleteapprovedgrn')==1)
					{
						$html .= "<a href='deleteapprovedgrn/{$detail_id}' class='btn btn-danger btn-clean'><i class='icon-pencil'></i> Delete</a>";
					}
					if(self::retrive_accessrights($role,'updategrnapproved')==1)
					{					
					$html .= "<a href='updategrn/{$uid}/approved' class='btn btn-success btn-clean'><i class='icon-pencil'></i> Edit</a>";
					}
					if(self::retrive_accessrights($role,'previewapprovedgrn')==1)
					{
					$html .= "<a href='previewapprovedgrn/{$uid}' target='_blank' class='btn btn-primary btn-clean'><i class='icon-pencil'></i> View</a>";
					}

					if($changes_status == 1)
					{
						if(self::retrive_accessrights($role,'auditgrnchanges')==1)
						{
							$html .= "<a href='auditgrnchanges/{$uid}' target='_blank' class='btn btn-primary btn-clean'><i class='icon-pencil'></i> Changes</a>";
						}
					}
					$row[ $column['dt'] ] =$html;
				}
	...
	{	
		...
		$or["grn.project_id IN"] = (!empty($post["pro_id"]) && $post["pro_id"][0] != "All" )?$post["pro_id"]:NULL;
		$or["grn.payment_method ="] = (!empty($post["payment_mod"]))?$post["payment_mod"]:NULL;
		$or["grn_detail.material_id IN"] = (!empty($post["material_id"]) && $post["material_id"][0] != "All")?$post["material_id"]:NULL;
		$or["grn.vendor_userid IN"] = (!empty($post["vendor_userid"]) && $post["vendor_userid"][0] != "All")?$post["vendor_userid"]:NULL;
		$or["grn.grn_date >="] = ($post["date_from"] != "")?date("Y-m-d",strtotime($post["date_from"])):NULL;
		$or["grn.grn_date <="] = ($post["date_to"] != "")?date("Y-m-d",strtotime($post["date_to"])):NULL;
		$or["grn.grn_no ="] = (!empty($post["grn_no"]))?$post["grn_no"]:NULL;
		$or["grn.challan_no ="] = (!empty($post["challan_no"]))?$post["challan_no"]:NULL;
		
		if($or["grn.project_id IN"] == NULL)
		{
			if(self::project_alloted($role)==1)
			{ 
				$or["grn.project_id IN"] = implode(",",$projects_ids);
			}
		}
		...
		{
			// $or["grn.manualpo_no"] = 0; 
			// $or["grn.local_po_id"] = 0;
			$or["grn.po_id !="] = "";
		...
		foreach($or as $key=>$value)
		{
			if($i == 0)
			{
				if($key == "grn.project_id IN")
				{
					$extraquery .= ' '. $key .' '. '('.$value.')';
					// $extraquery .= ' '. $key .' '. '('.implode(",",$value).')';
				}elseif($key == "grn_detail.material_id IN")
				{
					$extraquery .= ' '. $key .' '. '('.$value.')';
				}elseif($key == "grn.vendor_userid IN")
				{
					$extraquery .= ' '. $key .' '. '('.$value.')';
				}else
				{
