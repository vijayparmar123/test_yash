...
class SSP_inventoryrmc {
	...
	static function data_output ( $columns, $data ,$user)
	{
		...
		for ( $i=0, $ien=count($data) ; $i<$ien ; $i++ ) {
			...
			for ( $j=0, $jen=count($columns) ; $j<$jen ; $j++ ) {
				...
				{
					$uid=$data[$i][ $columns[$j]['field'] ];
					$concrete_grade = self::get_concrete_grade_name($uid);
					$row[ $column['dt'] ] = $concrete_grade ;
				}
				else if($j== 11)
				{
					$uid=$data[$i][ $columns[$j]['field'] ];
					...
					$html .= "<a href='viewinventoryrmc/{$uid}' target='_blank' class='btn btn-primary btn-clean'><i class='icon-pencil'></i> View</a>";
					
					if(self::retrive_accessrights($role,'unapprovedrmc')==1)
					{
						$html .= "<a href='unapprovedrmc/{$uid}' class='btn btn-danger btn-clean'><i class='icon-pencil'></i> Delete</a>";
					}
	...
	{	
		...
		$or["rmc.project_id ="] = (!empty($post["pro_id"]) && $post["pro_id"][0] != "All" )?$post["pro_id"]:NULL;
		$or["rmc.rmc_date >="] = ($post["date_from"] != "")?date("Y-m-d",strtotime($post["date_from"])):NULL;
		$or["rmc.rmc_date <="] = ($post["date_to"] != "")?date("Y-m-d",strtotime($post["date_to"])):NULL;
		
		$or["rmc.rmc_no ="] = (!empty($post["rmc_no"]))?$post["rmc_no"]:NULL;
		$or["rmc.concrete_grade ="] = (!empty($post["concrete_grade"]) && $post["concrete_grade"][0] != "All")?$post["concrete_grade"]:NULL;
		$or["asset.asset_id ="] = (!empty($post["asset_id"]) && $post["asset_id"][0] != "All")?$post["asset_id"]:NULL;
	...
	static function get_concrete_grade_name($id)
	{
