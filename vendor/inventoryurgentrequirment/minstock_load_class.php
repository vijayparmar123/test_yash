...
class SSP_records {
		...
		for ( $i=0, $ien=count($data) ; $i<$ien ; $i++ ) {
			...
			for ( $j=0, $jen=count($columns) ; $j<$jen ; $j++ ) {
				...
				else if($j== 12)
				{
					...
					$min_stock_level = self::get_min_stock_level($project_id,$m_id);
					$row[ $column['dt'] ] = $min_stock_level;
				}
				else if($j== 13)
				{
					$uid=$data[$i][ $columns[$j]['field'] ];
				...
				else if($j== 14)
				{
					...
					if(self::retrive_accessrights($role,'managestockurgent')==1)
					{
			...
			// if($min_stock_level > $current_stock)
			// {
				// debug('hhh');
				$out[] = $row;
			// }
	...
	{	
		$stock_id = self::get_urgent_required_material_id();
		if(empty($stock_id))
		{
			$stock_id = [0];
		}
		...
		$or["stock_history.stock_id IN"] = implode(",",$stock_id);
		...
		//material id and project id which has stock less then min stock level
		// $allow_id = json_decode(self::get_urgent_required_material_id());
		// $allow_project_id = $allow_id->project_id;
		// $allow_material_id = $allow_id->material_id;
		// $filter_project_id = $or["stock_history.project_id IN"];
		// $filter_material_id = $or["stock_history.material_id IN"];
		
		// $filter_project_id = explode(",",$filter_project_id);
		// $filter_material_id = explode(",",$filter_material_id);
		
		// if($or["stock_history.project_id IN"] == "")
		// {
			// $or["stock_history.project_id IN"] = implode(",",$allow_project_id);
		// }else{
			// $result = array_intersect($allow_project_id, $filter_project_id);
			// $or["stock_history.project_id IN"] = implode(",",$result);
		// }
		
		// if($or["stock_history.material_id IN"] == "")
		// {
			// $or["stock_history.material_id IN"] = implode(",",$allow_material_id);
		// }else{
			// $result = array_intersect($allow_material_id, $filter_material_id);
			// $or["stock_history.material_id IN"] = implode(",",$result);
		// }
		...
		foreach($or as $key=>$value)
		{
			if($i == 0)
			{
				...
				{
					$extraquery .= ' '. $key .' '. '("'.$value.'")';
				}else if($key == "stock_history.stock_id IN")
				{
					$extraquery .= ' '. $key .' '. '('.$value.')';
				}else
				{
	...
	static function get_urgent_required_material_id()
	{
		$erp_stock_history = TableRegistry::get('erp_stock_history');
		$results = $erp_stock_history->find()->group(["project_id","material_id","material_name"]);
		$ids = array();
		foreach($results as $material)
		{
			$project_id = $material->project_id;
			$material_id = $material->material_id;
			$current_stock = self::get_current_stock($project_id,$material_id);
			$min_level_stock = self::get_min_stock_level($project_id,$material_id);
			if($current_stock < $min_level_stock)
			{
				$ids['material_id'][] = $material->material_id;
				$ids['project_id'][] = $material->project_id;
			}
		}
		
		$history_id = array();
		foreach($ids['project_id'] as $key=>$value)
		{
			$project_id = $ids['project_id'][$key];
			$material_id = $ids['material_id'][$key];
			
			
			$records = $erp_stock_history->find()->where(["project_id"=>$project_id,"material_id"=>$material_id]);
			foreach($records as $record)
			{
				$history_id[] = $record->stock_id;
			}
		}
