		...
		for ( $i=0, $ien=count($data) ; $i<$ien ; $i++ ) {
			...
			for ( $j=0, $jen=count($columns) ; $j<$jen ; $j++ ) {
				...
				else if($j== 4)
				{
					...
					$party_type = $data[$i][ $columns[14]['field'] ];
					$uid = $data[$i][ $columns[15]['field'] ];
					...
					// $uid=$data[$i][ $columns[$j]['field'] ];
					// $party_type = $data[$i][ $columns[12]['field'] ];
					// $new_party_name = $data[$i][ $columns[13]['field'] ];
					// $is_agency = strpos($uid,"NEC");
				...
				else if($j== 9)
				{
					...
					else if($uid == "completed" ){
						$pay_status = "Paid";
					}
					else if($uid == "Check with A/C Dept." ){
						$pay_status = "Check with A/C Dept.";
					}
					$row[ $column['dt'] ] = $pay_status;
				}
				else if($j== 10)
				{
					$uid=$data[$i][ $columns[$j]['field'] ];
					$action_by = self::get_action_by($uid);
					 $row[ $column['dt'] ] = $action_by;
				}
				else if($j== 13)
				{
					$uid=$data[$i][ $columns[$j]['field'] ];
					$payStatus = $data[$i][ $columns[9]['field'] ];
					$role = self::get_user_role($user);
					$html = '';
					
					if(self::retrive_accessrights($role,'billdisapprove') == 1)
					{
						if($payStatus == "pending"){
							$html .= "<a href='delete/{$uid}' class='btn btn-danger btn-clean cancelpo'><i class='icon-pencil'></i> Delete</a>";
						}else {
							$html .= "<a href='billdisapprove/{$uid}' class='btn btn-danger btn-clean'><i class='icon-pencil'></i> Delete</a>";
						}
					}
					if(self::retrive_accessrights($role,'updatebill') == 1){
						$html .= "<a href='addinwardbill/{$uid}' class='btn btn-success btn-clean'><i class='icon-pencil'></i> Update</a>";
					}
				...
				else if($j== 14)
				{
					$uid=$data[$i][ $columns[$j]['field'] ];
					$html = '';
					if($role == "erphead" || $role == "accounthead" || $role == "senioraccountant")
					{					
...
$or["inward.project_id IN"] = ($request["project_id"] != "" && $request["project_id"] != "all")?$request["project_id"]:NULL;
...
if(!empty($request["pay_status"]) && $request["pay_status"] != "All")
{
	...
	{
		$or["inward.status_inward ="] = $request["pay_status"];
	}
	else
	{
		$or["inward.status_inward !="] = 'pending';
	}
}
else
{
	$or["inward.status_inward ="] = NULL;
}
...
exit;*/
	...
	{
		$ag_tbl = TableRegistry::get("erp_vendor");
		$agency = $ag_tbl->find()->where(["user_id"=>$code])->hydrate(false)->toArray();
		if(!empty($agency))
		{
			return $agency[0]["vendor_name"];
		}else{
			return "-";
		}
	}
	...
	static function get_action_by($inward_bill_id)
	{
		$erp_inward_bill = TableRegistry::get('erp_inward_bill');
		$record = $erp_inward_bill->get($inward_bill_id);
		$status = $record->status_inward;
		$action_by  = '';
		if($status == 'completed')
		{
			$action_by  = $record->accept_by;
		}
		if($status == 'Check with A/C Dept.')
		{
			$action_by  = $record->accept_by;
		}
		else if($status == 'accept')
		{
			$action_by  = $record->pending_approve_by;
		}
		else if($status == 'checked')
		{
			$action_by  = $record->checked_by;
		}
		else
		{
			$action_by  = $record->created_by;
		}
		return self::get_user_name($action_by);
	}
