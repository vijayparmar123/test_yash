		...
		for ( $i=0, $ien=count($data) ; $i<$ien ; $i++ ) {
			...
			for ( $j=0, $jen=count($columns) ; $j<$jen ; $j++ ) {
				...
				else if($j== 12)
				{
					...
					if(self::retrive_accessrights($role,'manualentry')==1)
					{
						$html .= "<a href='javascript:void(0)' class='btn btn-primary btn-clean po_receivemanual_entry' data-id='{$po_detail_id}' data-toggle='modal' data-target='#load_modal'><i class='icon-eye-open'></i> Manual Entry</a>";
					}
					...
					if(self::retrive_accessrights($role,'postatus')==1)
					{
					...
					if(self::retrive_accessrights($role,'removepofromgrn')==1)
					{
						$html .= "<a href='removepofromgrn/{$po_detail_id}' class='btn btn-danger btn-clean'><i class='icon-pencil'></i> Delete</a>";
					}
