...
class SSP_planningWoNoRateRecords {
	/**
		...
		for ( $i=0, $ien=count($data) ; $i<$ien ; $i++ ) {
			...
			for ( $j=0, $jen=count($columns) ; $j<$jen ; $j++ ) {
				...
				else if($j== 4)
				{
					...
					if($is_agency == 1)
					{
						$party = $data[$i][ $columns[7]['field'] ];
					}else{
						$party = $data[$i][ $columns[8]['field'] ];
					}
					$row[ $column['dt'] ] = $party;
				}
				else if($j== 5)
				...
				else if($j== 6)
				{
					...
					if(self::retrive_accessrights($role,'wonorate')==1)
					{
						$html .= "<a href='planningpreviewapprovedwonorate/{$uid}' target='_blank' class='btn btn-primary btn-clean'><i class='icon-eye-open'></i> View</a>";
					}
									
					// if(self::retrive_accessrights($role,'cancelwo')==1)
					// {
						// $html .= "<a href='cancelwo/{$uid}' class='btn btn-danger btn-clean'><i class='icon-pencil'></i> Cancel W.O.</a>";
					// }
