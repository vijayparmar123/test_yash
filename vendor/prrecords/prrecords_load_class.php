...
class SSP_pr {
	...
	static function data_output ( $columns, $data ,$user)
	{
		...
		for ( $i=0, $ien=count($data) ; $i<$ien ; $i++ ) {
			...
			for ( $j=0, $jen=count($columns) ; $j<$jen ; $j++ ) {
				...
				{
					$uid=$data[$i][ $columns[$j]['field'] ];
					$attach_label=$data[$i][ $columns[6]['field'] ];
					$attached_files = json_decode($uid);	
					$attached_label = json_decode($attach_label);	
					$html = "";
				...
				else if($j== 5)
				{
					...
					$html .= "<a href='previewprapprove/{$uid}' target='_blank' class='btn btn-primary btn-clean'><i class='icon-pencil'></i> View</a>";
					
					if(self::retrive_accessrights($role,'unapprovepr')==1)
					{
						$html .= "<a href='unapprovepr/{$uid}' class='btn btn-danger btn-clean cancelmaintenance'><i class='icon-pencil'></i> Delete</a>";
					}
	...
	{	
		...
		$or = array();
		$orWhere = array();
		$orWhere["pr_material.approved_for_grnwithoutpo ="] = 1;
		$orWhere["pr_material.show_in_purchase IN"] = [1,3];
		
		$or["purchase_request.project_id IN"] = (!empty($post["pro_id"]) && $post["pro_id"][0] != "All" )?$post["pro_id"]:NULL;
		$or["pr_material.material_id IN"] = (!empty($post["material_id"]) && $post["material_id"][0] != "All")?$post["material_id"]:NULL;
		$or["purchase_request.pr_date >="] = ($post["date_from"] != "")?date("Y-m-d",strtotime($post["date_from"])):NULL;
		$or["purchase_request.pr_date <="] = ($post["date_to"] != "")?date("Y-m-d",strtotime($post["date_to"])):NULL;
		$or["purchase_request.prno ="] = (!empty($post["pr_no"]))?$post["pr_no"]:NULL;
		
		if($or["purchase_request.project_id IN"] == NULL)
		{
			if(self::project_alloted($role)==1)
			{ 
				$or["purchase_request.project_id IN"] = implode(",",$projects_ids);
			}
		}
		
		if($or["pr_material.material_id IN"] == NULL)
		{
			if($role == "deputymanagerelectric")
			{
				$material_ids = self::get_deputymanagerelectric_material();
				$material_ids = json_decode($material_ids);
				$or["pr_material.material_id IN"] = implode(",",$material_ids);
			}
		...
		{
			$or["pr_material.show_in_purchase ="] = "1";
		}
		else if($post["purchase_mod"] == "local")
		{
			$or["pr_material.show_in_purchase ="] = "0";
		...
		foreach($or as $key=>$value)
		{
			if($i == 0)
			{
				if($key == "purchase_request.project_id IN")
				{
					$extraquery .= ' '. $key .' '. '('.$value.')';
					// $extraquery .= ' '. $key .' '. '('.implode(",",$value).')';
				}elseif($key == "pr_material.material_id IN")
				{
					$extraquery .= ' '. $key .' '. '('.$value.')';
				}else
				{
			...
			{
				if($key == "maintenance.project_id IN")
				{
		...
		foreach($orWhere as $key=>$value)
		{
			if($ii == 0)
			{
				if($key == "pr_material.show_in_purchase IN")
				{
					$orwherequery .= ' '. $key .' '. '("'.implode('","',$value).'")';
				}else{
					$orwherequery .= ' '. $key .' '. "'$value'";
				}
			}else{
				if($key == "pr_material.show_in_purchase IN")
				{
					// $orwherequery .= ' '.'OR' .' '. $key .' '. "'$value'";
					$orwherequery .= ' '.'OR' .' '. $key .' '. '("'.implode('","',$value).'")';
				}else{
					$orwherequery .= ' '.'OR' .' '. $key .' '. "'$value'";
				}
			}
		...
		if($orwherequery != '')
		{
			$extrawhere .= ($extrawhere != '' || $where != '') ? 'AND ('.$orwherequery.')' : 'WHERE ('. $orwherequery.')';
		}
		
		$group_by = "GROUP BY purchase_request.prno";
