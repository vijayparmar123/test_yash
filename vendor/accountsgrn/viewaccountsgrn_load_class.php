...
class SSP_accountgrn {
			...
			for ( $j=0, $jen=count($columns) ; $j<$jen ; $j++ ) {
				...
				else if($j== 5)
				{
					...
					if($uid)
					{
						...
						$material = $data[$i][ $columns[12]['field'] ];
					}
				...
				else if($j== 8)
				{
					...
					if($uid)
					{
						$code = self::get_items_units($uid);
					}else{
						$code = $data[$i][ $columns[11]['field'] ];
					}
					$row[ $column['dt'] ] = $code ;
				}
				else if($j== 9)
				{
					$grn_id=$data[$i][ $columns[$j]['field'] ];
					$html = '';
					
					$html .= "<a href='accountpreviewgrn/{$grn_id}' target='_blank' class='btn btn-primary btn-clean'><i class='icon-eye-open'></i> View</a>";
				...
				{
					$grn_id=$data[$i][ $columns[$j]['field'] ];
					$project_id=$data[$i][ $columns[14]['field'] ];
					...
					$html .= "<div class='checkbox'>
										...
										value='{$grn_id}' name='Approve'/> </label>
									</div>";
					$row[ $column['dt'] ] =$html;
				}
				else {
	...
	{	
		...
		$or["grn.payment_method ="] = (!empty($post["payment_mod"]) && $post["payment_mod"] != "All")?$post["payment_mod"]:NULL;
		$or["grn_detail.material_id ="] = (!empty($post["material_id"]) && $post["material_id"] != "All")?$post["material_id"]:NULL;
		$or["grn.vendor_userid ="] = (!empty($post["party_id"]) && $post["party_id"] != "All")?$post["party_id"]:NULL;
		...
		$or["grn.approved_status ="] = 1;
		$or["grn.show_in_account ="] = 0;
		// echo "<pre>";
		// print_r($post);
		// echo "</pre>";
		// echo "<pre>";
