			...
			for ( $j=0, $jen=count($columns) ; $j<$jen ; $j++ ) {
				...
				else if($j== 8)
				{
					...
					if(is_numeric($material_id))
					{
						...
						$static_unit=$data[$i][ $columns[11]['field'] ];
						$unit = $static_unit;
					}
				...
				else if($j== 10)
				{
					...
					if(self::retrive_accessrights($role,'ponorate')==1)
					{
						if(date('Y-m-d',strtotime($po_date)) > date('Y-m-d',strtotime('01-07-2017')))
						{
							$html .= "<a href='previewpo2norate/{$uid}' target='_blank' class='btn btn-primary btn-clean'><i class='icon-eye-open'></i> View</a>";
						}else{
