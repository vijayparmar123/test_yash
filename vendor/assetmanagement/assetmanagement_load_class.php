		...
		for ( $i=0, $ien=count($data) ; $i<$ien ; $i++ ) {
			...
			for ( $j=0, $jen=count($columns) ; $j<$jen ; $j++ ) {
				...
				else if($j== 1)
				{
					...
					$asset_group = self::get_asset_group_name($uid);
					$row[ $column['dt'] ] = $asset_group;
				}
				else if($j== 5)
				{
					$uid=$data[$i][ $columns[$j]['field'] ];
				...
				{
					$uid=$data[$i][ $columns[$j]['field'] ];
					$issued_to = self::get_asset_last_issueto($uid);
					$row[ $column['dt'] ] = $issued_to;
				}
				else if($j== 10)
				{
					$uid=$data[$i][ $columns[$j]['field'] ];
					$release_date = self::get_asset_release_date($uid);
					$row[ $column['dt'] ] = $release_date;
				}
				else if($j== 11)
				{
					$uid=$data[$i][ $columns[$j]['field'] ];
					...
					$html .= "<a href='viewasset/{$uid}' target='_blank' class='btn btn-primary btn-clean'><i class='icon-eye-open'></i> View</a>";
					
					if(self::retrive_accessrights($role,'addasset')==1)
					{
						$html .= "<a href='add/{$uid}' class='btn btn-primary btn-clean'><i class='icon-pencil'></i> Edit</a>";
					}
				...
				{
					$uid=$data[$i][ $columns[$j]['field'] ];
					$deployed_to=$data[$i][ $columns[8]['field'] ];
					$role = self::get_user_role($user);
					$html1 = '';
										
					if(self::retrive_accessrights($role,'isseuasset')==1)
					{
						if($role =='constructionmanager' || $role =='billingengineer' || $role == 'assistantpmm')
						{
							$user_project_ids = self::users_project($user);
							if(in_array($deployed_to,$user_project_ids))
							{
								$html1 .= '<button type="button"  id="issueasset" data-type="issueasset"  class="btn btn-info issueviewmodal btn-clean" asset_id="'.$uid.'"> <i class="icon-random"></i>Issued To</button>';
							}
						}else{
							$html1 .= '<button type="button"  id="issueasset" data-type="issueasset"  class="btn btn-info issueviewmodal btn-clean" asset_id="'.$uid.'"> <i class="icon-random"></i>Issued To</button>';
						}
				...
				else if($j== 13)
				{
					...
					if(self::retrive_accessrights($role,'bookingassest')==1)
					{
						$html2 .= '<button type="button" id="bookasset" data-type="bookasset" data-toggle="modal" data-target="#asset_booking_load_modal" class="btn btn-info bookingviewmodal btn-clean" asset_id="'.$uid.'"> <i class="icon-random"></i>Booking </button>';
					}
				...
				else if($j== 14)
				{
					...
					$role = self::get_user_role($user);
					$deployed_to=$data[$i][ $columns[8]['field'] ];
					$quantity=$data[$i][ $columns[8]['field'] ];
					$html3 = '';
										
					if(self::retrive_accessrights($role,'transferasset')==1)
					{
						
						$accept_status = self::is_asset_accept_remain($uid);
						if(!$accept_status){
							if($role == 'constructionmanager')
							{
								$allow_projects = self::users_project($user);
								if(in_array($deployed_to,$allow_projects))
								{
								$html3 .= '<button type="button"  id="transfereasset" data-type="transfereasset" data-toggle="modal" 
								data-target="#load_modal" class="btn btn-info viewmodal btn-clean" asset_id="'.$uid.'" quantity="'.$quantity.'"> <i class="icon-random"></i>Transfer </button>';
								}
							}else{
								$html3 .= '<button type="button"  id="transfereasset" data-type="transfereasset" data-toggle="modal" 
								data-target="#load_modal" class="btn btn-info viewmodal btn-clean" asset_id="'.$uid.'" quantity="'.$quantity.'"> <i class="icon-random"></i>Transfer </button>';
				...
				else if($j== 15)
				{
					...
					if(self::retrive_accessrights($role,'acceptasset')==1)
					{
						$accept_status = self::is_asset_accept_remain($uid);
						if($accept_status)
						{
							if($role == 'constructionmanager')
							{
								$user_project = self::users_project($user);
								$transfer_to_project = self::get_asset_last_transfer_project($uid);
								if(in_array($transfer_to_project,$user_project))
								{
									$html4 .= '<button type="button"  id="acceptasset" data-type="acceptasset" data-toggle="modal" 
									data-target="#load_modal1" class="btn btn-info viewmodalaccept btn-clean" asset_id="'.$uid.'" quantity="'.$quantity.'"> <i class="icon-random"></i>Accept </button>';
								}
							}else{
								$html4 .= '<button type="button"  id="acceptasset" data-type="acceptasset" data-toggle="modal" 
								data-target="#load_modal1" class="btn btn-info viewmodalaccept btn-clean" asset_id="'.$uid.'" quantity="'.$quantity.'"> <i class="icon-random"></i>Accept </button>';
	...
	{	
		...
		if($or["asset.deployed_to IN"] == NULL)
		{
			if(self::project_alloted($role)==1){ 
				$or["asset.deployed_to IN"] = implode(",",$projects_ids);
			}
		}
