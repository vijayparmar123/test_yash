...
class SSP_Accept {
	...
	static function data_output ( $columns, $data ,$user)
	{
		...
		for ( $i=0, $ien=count($data) ; $i<$ien ; $i++ ) {
			...
			for ( $j=0, $jen=count($columns) ; $j<$jen ; $j++ ) {
				...
				{
					$new_party_name=$data[$i][ $columns[$j]['field'] ];
					// debug($new_party_name);
					$party_type = $data[$i][ $columns[16]['field'] ];
					// debug($party_type);
					$uid = $data[$i][ $columns[17]['field'] ];
					// debug($uid);die;
					...
					{
					    $ag_name = $data[$i][ $columns[11]['field'] ];
					}
					...
					{
						$ag_name = $data[$i][ $columns[12]['field'] ];			
					}
				...
				// 	$uid=$data[$i][ $columns[$j]['field'] ];
				// 	$date =  date('Y-m-d');
				// 	$bill_date = $data[$i][ $columns[5]['field'] ];
				...
				{
					$uid=$data[$i][ $columns[14]['field'] ];
					$inward_status=$data[$i][ $columns[18]['field'] ];
					$role = self::get_user_role($user);
					$html = '';
					
					if(self::retrive_accessrights($role,'addinwardbill')==1) {
						if($inward_status == 'approved'){
							$html .= "<a href='addinwardbill/{$uid}' style='display:none;' class='btn btn-primary btn-clean'><i class='icon-pencil'></i> Update</a>";
						}else {
							$html .= "<br><br> <a href='addinwardbill/{$uid}' class='btn btn-primary btn-clean'><i class='icon-pencil'></i> Update</a>";
						}
					}
					// if(self::retrive_accessrights($role,'deleteinwardbill')==1)
					// {
					// 	$html .= "<a href='delete/{$uid}' target='_blank' class='btn btn-danger btn-clean'><i class='icon-pencil'></i> Delete</a>";
					// }
					$html .= "<br><br><a href='viewbill/{$uid}' target='_blank' class='btn btn-primary btn-clean'><i class='icon-pencil'></i> View</a>";
					
					$html .= "<br><br><a data-toggle='modal' data-target='#load_modal' class='btn btn-info add_remarks' uid='{$uid}'>Remark</a>";

					$row[ $column['dt'] ] =$html;
				}
				else if($j== 11)
				{
					$uid=$data[$i][ $columns[13]['field'] ];
					// debug($uid);
					$inward_status=$data[$i][ $columns[18]['field'] ];
					// debug($inward_status);die;
					$html = '';
					if(self::retrive_accessrights($role,'checkedinwardbill')==1)
					{					
						// $html .= "<input type='checkbox' name='cheking[]' value='cheking' class='cheking' dataid='".(($inward_status == 'checked' || $inward_status == 'approved')?'':$uid)."'
						// ".(($inward_status == 'checked' || $inward_status == 'approved')?'checked':'')." ".(($inward_status == 'checked' || $inward_status == 'approved')?'DISABLED':'').">";
						$html .= "<input type='checkbox' name='cheking[]' value='cheking' class='cheking' dataid='".(($inward_status == 'checked' || $inward_status == 'approved')?'':$uid)."'
						".(($inward_status == 'checked' || $inward_status == "approved")?'checked':'')." ".(($inward_status == 'checked' || $inward_status == 'approved')?'DISABLED':'').">";
					}
					$row[ $column['dt'] ] =$html;
				...
				else if($j== 12)
				{
					$uid=$data[$i][ $columns[13]['field'] ];
					$html = '';
					if(self::retrive_accessrights($role,'approveinwardbill')==1)
					{		
						// debug($inward_status);die;			
						// $html .= "<input type='checkbox' name='ch_approved[]' value='approve' class='approved' dataid='".(($inward_status == 'approved')?'':$uid)."' 
						// ".(($inward_status !== 'checked' || $inward_status == 'approved')?'DISABLED':'')." ".(($inward_status !== 'checked' || $inward_status == 'approved')?'checked':'').">";
						$html .= "<input type='checkbox' name='ch_approved[]' value='approve' class='approved' dataid='".(($inward_status == 'approved')?'':$uid)."' 
						".(($inward_status !== 'checked' && $inward_status !== 'accept')?'DISABLED':'')." ".(($inward_status !== 'checked' && $inward_status == "approved")?'checked':'').">";
					}
					$row[ $column['dt'] ] =$html;
				}
				else if($j== 13)
				{
					$uid=$data[$i][ $columns[$j]['field'] ];
					$html = '';		
					if(self::retrive_accessrights($role,'acceptinwardbill')==1)
					{				
						$html .= "<input type='checkbox' name='ch_accepted[]' value='accept' class='accepted' dataid='".(($inward_status == 'accepted')?'':$uid)."' 
						".(($inward_status == 'pending' || $inward_status == 'checked' || $inward_status !== 'approved')?'DISABLED':'')." ".(($inward_status == 'accepted')?'checked':'').">";
					}
					$row[ $column['dt'] ] =$html;
				}
				else if($j== 14)
				{
					$uid=$data[$i][ $columns[20]['field'] ];
					$html = '';
					$html .= $uid;
					$row[ $column['dt'] ] =$html;
				}
	...
	static function order ( $request, $columns )
	{
		...
		if ( isset($request['order']) && count($request['order']) ) {
			...
			$order = 'GROUP BY `inward_bill_id` ORDER BY '.implode(', ', $orderBy);
		}
...
$or["inward.date >="] = ($request["date_from"] != "")?date("Y-m-d",strtotime($request["date_from"])):NULL;
$or["inward.date <="] = ($request["date_to"] != "")?date("Y-m-d",strtotime($request["date_to"])):NULL;
$or["inward.bill_date >="] = ($request["bill_date_from"] != "")?date("Y-m-d",strtotime($request["bill_date_from"])):NULL;
$or["inward.bill_date <="] = ($request["bill_date_to"] != "")?date("Y-m-d",strtotime($request["bill_date_to"])):NULL;
$or["inward.project_id IN"] = (!empty($request["project_id"]) && $request["project_id"] != "all")?$request["project_id"]:NULL;
$or["inward.party_name ="] = (!empty($request["party_id"]) && $request["party_id"] != "All")?$request["party_id"]:NULL;
$or["inward.bill_type ="] = (!empty($request["bill_type"]) && $request["bill_type"] != "All" )?$request["bill_type"]:NULL;
$or["inward.payment_method ="] = (!empty($request["payment_mod"]) && $request["payment_mod"] != "All")?$request["payment_mod"]:NULL;
$or["inward.inward_bill_no ="] = (!empty($request["bill_no"]))?$request["bill_no"]:NULL;
$or["inward.invoice_no ="] = (!empty($request["invoice_no"]))?$request["invoice_no"]:NULL;
$or["inward.po_no ="] = (!empty($request["powono"]))?$request["powono"]:NULL;
			
			if($or["inward.project_id IN"] == NULL)
			{
				if(self::project_alloted($role)==1)
				{ 
					$or["inward.project_id IN"] = $projects_ids;
				}
			}
			else
			{
				$or["inward.project_id IN"] = explode(",",$or["inward.project_id IN"]);
			}
			...
			foreach ($keys as $k)
			{unset($or[$k]);}
			$or["inward.status_inward IN"] = explode(",",$status);
			// debug($or);die;
...
// AND $table.id = wp_usermeta.user_id AND $table.ID = ANY (SELECT user_id FROM wp_usermeta WHERE meta_key = 'wp_capabilities' AND meta_value RLIKE 'patient')";
		...
		foreach($or as $key=>$value)
		{
			if($i == 0)
			{
				...
				else if($key == "inward.status_inward IN")
				{
					$extraquery .= ' '. $key .' '. '('.implode(',',$value ).')';
				}
				else
		...
		// if($where != '')
		// {
			// $where .= "AND status_inward = 'pending'";
		// }
		// else
		// {
			// $where = "where status_inward = 'pending'";
		// }
		/*
