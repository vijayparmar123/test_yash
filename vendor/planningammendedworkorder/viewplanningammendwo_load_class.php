...
class SSP_pawo {
		...
		for ( $i=0, $ien=count($data) ; $i<$ien ; $i++ ) {
			...
			for ( $j=0, $jen=count($columns) ; $j<$jen ; $j++ ) {
				...
				else if($j== 4)
				{
					...
					if($is_agency == 1)
					{
						$party = $data[$i][ $columns[9]['field'] ];
					}else{
						$party = $data[$i][ $columns[10]['field'] ];
					}
					$row[ $column['dt'] ] = $party;
				}
				else if($j== 5)
				{
					$uid=$data[$i][ $columns[$j]['field'] ];
					$contract_type = self::get_contract_title($uid);
					$row[ $column['dt'] ] = $contract_type;
				}
				else if($j== 6)
				{
					$uid=$data[$i][ $columns[$j]['field'] ];
					$amount = round($uid, 2);
					$row[ $column['dt'] ] = $amount;
				}
				else if($j== 7)
				{
					$uid=$data[$i][ $columns[$j]['field'] ];
					$po_date=$data[$i][ $columns[2]['field'] ];
					$ammend_approve=$data[$i][ $columns[11]['field'] ];
					$detail_id='';
					$role = self::get_user_role($user);
					$html = '';
					
					if(self::retrive_accessrights($role,'planningworecords')==1)
					{
						if($ammend_approve == 1) {
							$html .= "<a href='previewapprovedplanningwo/{$uid}' target='_blank' class='btn btn-primary btn-clean'><i class='icon-eye-open'></i> View</a>";
						}
					}
									
					if(self::retrive_accessrights($role,'cancelplanningwo')==1)
					{
						$html .= "<a href='cancelplanningwo/{$uid}' class='btn btn-danger btn-clean'><i class='icon-pencil'></i> Cancel W.O.</a>";
					}
				...
				{
					$updated=$data[$i][ $columns[$j]['field'] ];
					$wo_id=$data[$i][ $columns[7]['field'] ];
					$ammend_approve=$data[$i][ $columns[11]['field'] ];
					$po_date=$data[$i][ $columns[2]['field'] ];
					// $detail_id='';
					// $role = self::get_user_role($user);
					...
					if(self::retrive_accessrights($role,'ammendworkorder')==1)
					{
						if($updated == 0)
						{
							$html .= "<a href='ammendworkorder/{$wo_id}' target='_blank' class='btn btn-primary btn-clean'><i class='icon-pencil'></i> Ammend</a>";
						}elseif($updated == 1 && $ammend_approve == 0){
							$html .= "<a href='ammendworkorder/{$wo_id}' target='_blank' class='btn btn-primary btn-clean'><i class='icon-pencil'></i> Ammend</a>";
							$html .= "<input type='checkbox' value='{$wo_id}' id='ammend_approve'>";
						}else{
	...
	{	
		...
		$or["wo.wo_date >="] = ($post["date_from"] != "")?date("Y-m-d",strtotime($post["date_from"])):NULL;
		$or["wo.wo_date <="] = ($post["date_to"] != "")?date("Y-m-d",strtotime($post["date_to"])):NULL;
		$or["wo.project_id IN"] = (!empty($post["pro_id"]) && $post["pro_id"] != "All" )?$post["pro_id"]:NULL;
		$or["wo.party_userid IN"] = (!empty($post["party_userid"]) && $post["party_userid"] != "All")?$post["party_userid"]:NULL;
		$or["wo.contract_type IN"] = (!empty($post["contract_type"]) && $post["contract_type"] != "All")?$post["contract_type"]:NULL;
		
		$or["wo.wo_no ="] = (!empty($post["wo_no"]))?$post["wo_no"]:NULL;
		$or["wo.payment_method ="] = (!empty($post["payment_method"]) && $post["payment_method"] != "All")?$post["payment_method"]:NULL;
		$or["wo.last_wo ="] = 1;
		

		if($or["wo.project_id IN"] == NULL)
		{
			if(self::project_alloted($role)==1)
			{
				$or["wo.project_id IN"] = implode(",",$projects_ids);
			}
		}
		...
		$or["wo.ammend_approve !="] = 1;
		$or['wo.updated ='] = 1;
		$or["wo_detail.approved ="] = 1;
		// echo "<pre>";
		// print_r($post);
		// echo "</pre>";
		// echo "<pre>";
		...
		foreach($or as $key=>$value)
		{
			if($i == 0)
			{
				if($key == "wo.project_id IN")
				{
					$extraquery .= ' '. $key .' '. '('.$value.')';
				}elseif($key == "wo.party_userid IN")
				{
					$value = explode(",",$value);
					$extraquery .= ' '. $key .' '. '("'.implode('","',$value).'")';
				}elseif($key == "wo.contract_type IN")
				{
					$extraquery .= ' '. $key .' '. '('.$value.')';
				}else
				{
		...
		// $group_by = "GROUP BY wo_detail.wo_id,wo.project_id,wo.party_userid";
		$group_by = "GROUP BY wo_detail.wo_id";
	...
	static function contract_type_list()
	{
	...
	static function get_contract_title($contract_id)
	{
		$contract = self::contract_type_list();
