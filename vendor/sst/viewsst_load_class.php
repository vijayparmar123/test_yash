...
class SSP_sst {
	...
	static function data_output ( $columns, $data ,$user)
	{
		...
		for ( $i=0, $ien=count($data) ; $i<$ien ; $i++ ) {
			...
			for ( $j=0, $jen=count($columns) ; $j<$jen ; $j++ ) {
				...
				{
					$uid=$data[$i][ $columns[$j]['field'] ];
					$detail_id = $data[$i][ $columns[11]['field'] ];
					$role = self::get_user_role($user);
					$html = '';
					
					if(self::retrive_accessrights($role,'unapprovesst')==1)
					{
						$html .= "<a href='unapprovesst/{$detail_id}' class='btn btn-danger btn-clean'><i class='icon-pencil'></i> Delete</a>";
					}
					if(self::retrive_accessrights($role,'previewapprovedsst')==1)
					{
						$html .= "<a href='previewapprovedsst/{$uid}' target='_blank' class='btn btn-primary btn-clean'><i class='icon-eye-open'></i> View</a>";
					}
					$row[ $column['dt'] ] =$html;
				}
				else {
	...
	{	
		...
		$or["sst.project_id IN"] = (!empty($post["from_project_id"]) && $post["from_project_id"] != "All" )?$post["from_project_id"]:NULL;
		$or["sst.transfer_to IN"] = (!empty($post["to_project_id"]) && $post["to_project_id"] != "All" )?$post["to_project_id"]:NULL;
		$or["sst_detail.material_id IN"] = (!empty($post["material_id"]) && $post["material_id"] != "All")?$post["material_id"]:NULL;
		$or["sst.sst_date >="] = ($post["date_from"] != "")?date("Y-m-d",strtotime($post["date_from"])):NULL;
		$or["sst.sst_date <="] = ($post["date_to"] != "")?date("Y-m-d",strtotime($post["date_to"])):NULL;
		$or["sst.sst_no ="] = (!empty($post["sst_no"]))?$post["sst_no"]:NULL;
		
		if($or["sst.project_id IN"] == NULL)
		{
			if(self::project_alloted($role)==1)
			{ 
				$or["sst.project_id IN"] = implode(",",$projects_ids);
			}
		}
		...
		$or["sst_detail.approved_site2 ="] = 1;
		// echo "<pre>";
		// print_r($post);
		// echo "</pre>";
		// echo "<pre>";
		...
		foreach($or as $key=>$value)
		{
			if($i == 0)
			{
				if($key == "sst.project_id IN")
				{
					$extraquery .= ' '. $key .' '. '('.$value.')';
					// $extraquery .= ' '. $key .' '. '('.implode(",",$value).')';
				}elseif($key == "sst.transfer_to IN")
				{
					$extraquery .= ' '. $key .' '. '('.$value.')';
				}elseif($key == "sst_detail.material_id IN")
				{
					$extraquery .= ' '. $key .' '. '('.$value.')';
				}else
				{
