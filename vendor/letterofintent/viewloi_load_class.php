...
class SSP_loi {
			...
			for ( $j=0, $jen=count($columns) ; $j<$jen ; $j++ ) {
				...
				else if($j== 8)
				{
					...
					if(is_numeric($material_id))
					{
						...
						$static_unit=$data[$i][ $columns[12]['field'] ];
						$unit = $static_unit;
					}
				...
				else if($j== 11)
				{
					...
					if(self::retrive_accessrights($role,'loirecords')==1)
					{
						$html .= "<a href='previewloi/{$uid}' target='_blank' class='btn btn-primary btn-clean'><i class='icon-eye-open'></i> View</a>";
					}
									
					if(self::retrive_accessrights($role,'cancelloi')==1)
								{
						$html .= "<a href='cancelloi/{$uid}' class='btn btn-danger btn-clean'><i class='icon-pencil'></i> Delete</a>";
					}
	...
	{	
		...
		$or["loi.project_id IN"] = (!empty($post["pro_id"]) && $post["pro_id"] != "All" )?$post["pro_id"]:NULL;
		$or["loi_detail.material_id IN"] = (!empty($post["material_id"]) && $post["material_id"] != "All")?$post["material_id"]:NULL;
		$or["loi_detail.brand_id IN"] = (!empty($post["brand_id"]) && $post["brand_id"] != "All")?$post["brand_id"]:NULL;
		$or["loi.vendor_userid IN"] = (!empty($post["vendor_userid"]) && $post["vendor_userid"] != "All")?$post["vendor_userid"]:NULL;
		$or["loi.loi_date >="] = ($post["date_from"] != "")?date("Y-m-d",strtotime($post["date_from"])):NULL;
		$or["loi.loi_date <="] = ($post["date_to"] != "")?date("Y-m-d",strtotime($post["date_to"])):NULL;
		$or["loi.loi_no ="] = (!empty($post["loi_no"]))?$post["loi_no"]:NULL;
		// $or["loi.po_purchase_type ="] = (!empty($post["po_type"]) && $post["po_type"] != "All")?$post["po_type"]:NULL;
		// $or["loi_detail.po_type ="] = (!empty($post["po_type"]) && $post["po_type"] != "All")?$post["po_type"]:NULL;
		
		if($or["loi.project_id IN"] == NULL)
		{
			if(self::project_alloted($role)==1)
			{
				$or["loi.project_id IN"] = implode(",",$projects_ids);
			}
		}
		...
		foreach($or as $key=>$value)
		{
			if($i == 0)
			{
				if($key == "loi.project_id IN")
				{
					$extraquery .= ' '. $key .' '. '('.$value.')';
				}elseif($key == "loi_detail.material_id IN")
				{
					$extraquery .= ' '. $key .' '. '('.$value.')';
				}elseif($key == "loi_detail.brand_id IN")
				{
					$extraquery .= ' '. $key .' '. '('.$value.')';
				}elseif($key == "loi.vendor_userid IN")
				{
					$extraquery .= ' '. $key .' '. '('.$value.')';
				}else
				{
