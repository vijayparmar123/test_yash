...
class SSP_rbn {
		...
		for ( $i=0, $ien=count($data) ; $i<$ien ; $i++ ) {
			...
			for ( $j=0, $jen=count($columns) ; $j<$jen ; $j++ ) {
				...
				else if($j== 3)
				{
					...
					$uid=$data[$i][ $columns[$j]['field'] ];
					// $vendor_name = self::get_vendor_name($uid);
					$row[ $column['dt'] ] = $uid;
				}
				else if($j== 5) {
					$uid=$data[$i][ $columns[$j]['field'] ];
					// $material_id = $data[$i][ $columns[8]['field'] ];
					// $material_title = self::get_material_title($uid);
					$row[ $column['dt'] ] = $uid;
				}
				else if($j== 6)
				{
				...
				else if($j== 10)
				{
					...
					if(self::retrive_accessrights($role,'editrbn')==1)
					{
						$html .= "<a href='editrbn/{$uid}' target='_blank' class='btn btn-primary btn-clean'><i class='icon-edit'></i> Edit</a>";
					}
					if(self::retrive_accessrights($role,'unapproverbn')==1)
					{					
					$html .= "<a href='unapproverbn/{$uid}' class='btn btn-danger btn-clean'><i class='icon-trash'></i> Delete</a>";
					}
					if(self::retrive_accessrights($role,'previewapprovedrbn')==1)
					{
					$html .= "<a href='previewapprovedrbn/{$uid}' target='_blank' class='btn btn-primary btn-clean'><i class='icon-pencil'></i> View</a>";
					}

					if($changes_status == 1)
					{
						if(self::retrive_accessrights($role,'auditrbnchanges')==1)
						{
							$html .= "<a href='auditrbnchanges/{$uid}' target='_blank' class='btn btn-primary btn-clean'><i class='icon-pencil'></i> Changes</a>";
						}
					}
					$row[ $column['dt'] ] =$html;
				}
	...
	{	
		...
		$or["rbn.project_id IN"] = (!empty($post["pro_id"]) && $post["pro_id"] != "All" )?$post["pro_id"]:NULL;
		$or["rbn_detail.material_id IN"] = (!empty($post["material_id"]) && $post["material_id"] != "All")?$post["material_id"]:NULL;
		$or["rbn.agency_name IN"] = (!empty($post["agency_id"]) && $post["agency_id"] != "All")?$post["agency_id"]:NULL;
		$or["rbn.rbn_date >="] = ($post["date_from"] != "")?date("Y-m-d",strtotime($post["date_from"])):NULL;
		$or["rbn.rbn_date <="] = ($post["date_to"] != "")?date("Y-m-d",strtotime($post["date_to"])):NULL;
		$or["rbn.rbn_no ="] = (!empty($post["rbn_no"]))?$post["rbn_no"]:NULL;
		
		if($or["rbn.project_id IN"] == NULL)
		{
			if(self::project_alloted($role)==1)
			{ 
				$or["rbn.project_id IN"] = implode(",",$projects_ids);
			}
		}
		...
		foreach($or as $key=>$value)
		{
			if($i == 0)
			{
				if($key == "rbn.project_id IN")
				{
					$extraquery .= ' '. $key .' '. '('.$value.')';
					// $extraquery .= ' '. $key .' '. '('.implode(",",$value).')';
				}elseif($key == "rbn_detail.material_id IN")
				{
					$extraquery .= ' '. $key .' '. '('.$value.')';
				}elseif($key == "rbn.agency_name IN")
				{
					$extraquery .= ' '. $key .' '. '('.$value.')';
				}else
				{
	...
	static function get_vendor_name($data)
	{
		$erp_vendor = TableRegistry::get('erp_vendor'); 
		$results = $erp_vendor->find()->where(['user_id'=>$data]);
		$vendor_name = ' - ';
		foreach($results as $retrive_data)
		{
			$vendor_name = $retrive_data['vendor_name'];					
		}
		return $vendor_name;
	}
	...
	static function get_material_title($material_id){
		...
		foreach($results as $retrive_data)
		{
			$mat_title = $retrive_data['material_title'];
			// if(!empty($mat_unitid)){
