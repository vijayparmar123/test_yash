...
class SSP_records {
		...
		for ( $i=0, $ien=count($data) ; $i<$ien ; $i++ ) {
			...
			for ( $j=0, $jen=count($columns) ; $j<$jen ; $j++ ) {
				...
				else if($j== 14)
				{
					...
					if(self::retrive_accessrights($role,'managestockover')==1)
					{
	...
	{	
		$stock_id = self::get_over_purchased_material_id();
		if(empty($stock_id))
		{
			$stock_id = [0];
		}
	...
	static function get_over_purchased_material_id()
	{
		$erp_stock_history = TableRegistry::get('erp_stock_history');
		...
		foreach($results as $material)
		{
			...
			$max_level_stock = self::get_maximum_stock_level($project_id,$material_id);
			if($current_stock > $max_level_stock)
			{
